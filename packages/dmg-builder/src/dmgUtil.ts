...
export async function attachAndExecute(dmgPath: string, readWrite: boolean, task: () => Promise<any>) {
  ...
  if (device == null) throw new Error(`Cannot mount: ${attachResult}`);
...
export async function computeBackground(packager: PlatformPackager<any>): Promise<string> {
  ...
  if (resourceList.includes("background.tiff")) return path.join(packager.buildResourcesDir, "background.tiff");

  if (resourceList.includes("background.png")) return path.join(packager.buildResourcesDir, "background.png");
