...
export async function getLicenseButtonsFile(packager: PlatformPackager<any>): Promise<LicenseButtonsFile[]> {
...
export async function getLicenseButtons(
  licenseButtonFiles: LicenseButtonsFile[],
  langWithRegion: string,
  id: number,
  name: string,
) {
  ...
  for (const item of licenseButtonFiles) {
    if (item.langWithRegion !== langWithRegion) continue;

    try {
      const fileData = safeLoad(await readFile(item.file, "utf-8"));
      const buttonsStr =
      ...
      if (log.isDebugEnabled) log.debug({ lang: item.langName, data }, `overwriting license buttons`);
...
function getMacCodePage(lang: string, langWithRegion: string) {
  switch (lang) {
    ...
    case "zh": // chinese
      if (langWithRegion === "zh_CN") return ["gb2312"]; // Apple Simplified Chinese (GB 2312)
...
function getMacHexCode(str: string, i: number, macCodePages: any) {
  ...
  if (code < 128) return code.toString(16);

  if (code < 256) return iconv.encode(str[i], "macroman").toString("hex");
