...
export abstract class Publisher {
  ...
  protected createProgressBar(fileName: string, size: number): ProgressBar | null {
    ...
    if (this.context.progress == null || size < 512 * 1024) return null;

    return this.context.progress.createBar(
      `${" ".repeat(PADDING + 2)}[:bar] :percent :etas | ${chalk.green(fileName)} to ${this.providerName}`,
      { total: size, ...progressBarOptions },
    );
  }
  ...
  protected createReadStreamAndProgressBar(
    ...
    if (progressBar == null) return fileInputStream;

    const progressStream = new ProgressCallbackTransform(fileStat.size, this.context.cancellationToken, (it) =>
      progressBar.tick(it.delta),
    );
