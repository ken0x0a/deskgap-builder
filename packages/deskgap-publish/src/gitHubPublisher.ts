...
export class GitHubPublisher extends HttpPublisher {
  readonly _release = new Lazy(() =>
    this.token === "__test__" ? Promise.resolve(null as any) : this.getOrCreateRelease(),
  );
  ...
  constructor(
    ...
    if (isEmptyOrSpaces(token)) {
      ...
      if (!isTokenCharValid(token))
        throw new InvalidConfigurationError(
          `GitHub Personal Access Token (${JSON.stringify(
            token,
          )}) contains invalid characters, please check env "GH_TOKEN"`,
        );
    }

    this.token = token;

    if (version.startsWith("v")) throw new InvalidConfigurationError(`Version must not start with "v": ${version}`);
    ...
    if (isEnvTrue(process.env.EP_DRAFT)) {
      ...
      isEnvTrue(process.env.EP_PRE_RELEASE) ||
      isEnvTrue(process.env.EP_PRELEASE) /* https://github.com/deskgap-userland/deskgap-builder/issues/2878 */
    ) {
  ...
  async deleteRelease(): Promise<any> {
    if (!this._release.hasValue) return;

    const release = await this._release.value;
    for (let i = 0; i < 3; i++)
      try {
        return await this.githubRequest(
          `/repos/${this.info.owner}/${this.info.repo}/releases/${release.id}`,
          this.token,
          null,
          "DELETE",
        );
        ...
        if (e instanceof HttpError) {
          ...
          if (e.statusCode === 405 || e.statusCode === 502) continue;
        }
  ...
  async getRelease(): Promise<any> {
    return this.githubRequest<Release>(
      `/repos/${this.info.owner}/${this.info.repo}/releases/${(await this._release.value)!.id}`,
      this.token,
    );
  }
    ...
    return httpExecutor
      ...
      .catch((e) => {
        if (
          ...
          return this.overwriteArtifact(fileName, release).then(() =>
            this.doUploadFile(attemptNumber, parsedUrl, fileName, dataLength, requestProcessor, release),
          );

        if (attemptNumber > 3) return Promise.reject(e);
  ...
  private async getOrCreateRelease(): Promise<Release | null> {
    ...
    const releases = await this.githubRequest<Release[]>(
      `/repos/${this.info.owner}/${this.info.repo}/releases`,
      this.token,
    );
    for (const release of releases) {
      ...
      // https://github.com/deskgap-userland/deskgap-builder/issues/1197
      // https://github.com/deskgap-userland/deskgap-builder/issues/2072
      ...
      // https://github.com/deskgap-userland/deskgap-builder/issues/1133
      // https://github.com/deskgap-userland/deskgap-builder/issues/2074
      ...
      if (publishedAt != null && Date.now() - publishedAt > 2 * 3600 * 1000) {
        // https://github.com/deskgap-userland/deskgap-builder/issues/1183#issuecomment-275867187
        this.releaseLogFields = {
    ...
    // https://github.com/deskgap-userland/deskgap-builder/issues/1835
        ...
        configureRequestOptions(
          {
            ...
            path:
              this.info.host != null && this.info.host !== "github.com"
                ? `/api/v3${path.startsWith("/") ? path : `/${path}`}`
                : path,
  ...
  private async overwriteArtifact(fileName: string, release: Release) {
    ...
    const assets = await this.githubRequest<Asset[]>(
      `/repos/${this.info.owner}/${this.info.repo}/releases/${release.id}/assets`,
      this.token,
      null,
    );
    for (const asset of assets)
      if (asset.name === fileName) {
        await this.githubRequest<void>(
          `/repos/${this.info.owner}/${this.info.repo}/releases/assets/${asset.id}`,
          this.token,
          null,
          "DELETE",
        );
