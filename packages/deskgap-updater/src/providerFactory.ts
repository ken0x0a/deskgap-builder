...
export function createClient(
  ...
  switch (provider) {
    case "github": {
      ...
      const token =
        (githubOptions.private ? process.env.GH_TOKEN || process.env.GITHUB_TOKEN : null) || githubOptions.token;
      if (token == null) return new GitHubProvider(githubOptions, updater, runtimeOptions);
    ...
    case "generic": {
      ...
      return new GenericProvider(options, updater, {
        ...
        isUseMultipleRangeRequest:
          options.useMultipleRangeRequest !== false && isUrlProbablySupportMultiRangeRequests(options.url),
      });
    }
