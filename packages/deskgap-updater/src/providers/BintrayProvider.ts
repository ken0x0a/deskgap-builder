...
export class BintrayProvider extends Provider<UpdateInfo> {
  ...
  async getLatestVersion(): Promise<UpdateInfo> {
    try {
      ...
      const channelFile = files.find(
        (it) => it.name.endsWith(`_${channelFilename}`) || it.name.endsWith(`-${channelFilename}`),
      );
      if (channelFile == null)
      ...
      const channelFileUrl = new URL(
        `https://dl.bintray.com/${this.client.owner}/${this.client.repo}/${channelFile.name}`,
      );
