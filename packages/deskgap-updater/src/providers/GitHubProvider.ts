...
export abstract class BaseGitHubProvider<T extends UpdateInfo> extends Provider<T> {
  ...
  protected computeGithubBasePath(result: string): string {
    // https://github.com/deskgap-userland/deskgap-builder/issues/1903#issuecomment-320881211
...
export class GitHubProvider extends BaseGitHubProvider<UpdateInfo> {
  ...
  async getLatestVersion(): Promise<UpdateInfo> {
    ...
    if (version == null) throw newError(`No published versions on GitHub`, "ERR_UPDATER_NO_PUBLISHED_VERSIONS");
  ...
  resolveFiles(updateInfo: UpdateInfo): ResolvedUpdateFileInfo[] {
    ...
    return resolveFiles(updateInfo, this.baseUrl, (p) =>
      this.getBaseDownloadPath(updateInfo.version, p.replace(/ /g, "-")),
    );
  }
...
export function computeReleaseNotes(
  ...
  feed: XElement,
  latestRelease: any,
): string | ReleaseNoteInfo[] | null {
  if (!isFullChangelog) return getNoteValue(latestRelease);

  const releaseNotes: ReleaseNoteInfo[] = [];
