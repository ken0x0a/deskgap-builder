...
export class NsisUpdater extends BaseUpdater {
  ...
  protected doInstall(options: InstallOptions): boolean {
    ...
    _spawn(options.installerPath, args).catch((e: Error) => {
      // https://github.com/deskgap-userland/deskgap-builder/issues/1129
    ...
    try {
      ...
      const oldBlockMapUrl = newUrlFromBase(
        `${fileInfo.url.pathname.replace(
          new RegExp(downloadUpdateOptions.updateInfoAndProvider.info.version, "g"),
          this.app.version,
        )}.blockmap`,
        fileInfo.url,
      );
 ...
 */
async function _spawn(exe: string, args: string[]): Promise<any> {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (process.pid !== undefined) resolve(true);
    } catch (error) {
      reject(error);
    }
  });
