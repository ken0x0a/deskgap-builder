...
export function verifySignature(
  ...
  return new Promise<string | null>((resolve) => {
    // https://github.com/deskgap-userland/deskgap-builder/issues/2421
    // https://github.com/deskgap-userland/deskgap-builder/issues/2535
    execFile(
      "powershell.exe",
      [
        "-NoProfile",
      ...
      (error, stdout, stderr) => {
        try {
          ...
          const result = `publisherNames: ${publisherNames.join(" | ")}, raw info: ${JSON.stringify(
            data,
...
function handleError(logger: Logger, error: Error | null, stderr: string | null): void {
  ...
  try {
    execFileSync("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", "ConvertTo-Json test"], {
      timeout: 10 * 1000,
    });
  } catch (testError) {
    logger.warn(
