...
export abstract class DifferentialDownloader {
  ...
  protected doDownload(oldBlockMap: BlockMap, newBlockMap: BlockMap): Promise<any> {
    ...
    for (const operation of operations) {
      ...
      if (operation.kind === OperationKind.DOWNLOAD) downloadSize += length;
    ...
    logger.info(
      `Full: ${formatBytes(newSize)}, To download: ${formatBytes(downloadSize)} (${Math.round(
        downloadSize / (newSize / 100),
      )}%)`,
    );
  ...
  private async doDownloadFile(tasks: Operation[], fdList: OpenedFile[]): Promise<any> {
    ...
    await new Promise((resolve, reject) => {
      ...
      w = (index: number): void => {
        ...
        if (debug != null) debug(`download range: ${range}`);
        ...
        const request = this.httpExecutor.createRequest(requestOptions, (response) => {
          ...
          if (response.statusCode >= 400) reject(createHttpError(response));
  ...
  private downloadFile(tasks: Operation[]): Promise<any> {
    const fdList: OpenedFile[] = [];
