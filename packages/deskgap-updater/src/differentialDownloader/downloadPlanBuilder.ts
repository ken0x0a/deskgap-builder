...
export function computeOperations(oldBlockMap: BlockMap, newBlockMap: BlockMap, logger: Logger): Operation[] {
...
function validateAndAdd(operation: Operation, operations: Operation[], checksum: string, index: number): void {
  if (isValidateOperationRange && operations.length !== 0) {
    ...
    if (
      ...
      const min = [lastOperation.start, lastOperation.end, operation.start, operation.end].reduce((p, v) =>
        p < v ? p : v,
      );
      throw new Error(
        `operation (block index: ${index}, checksum: ${checksum}, kind: ${
          ...
          `rel: ${lastOperation.start - min} until ${lastOperation.end - min} and ${operation.start - min} until ${
            operation.end - min
          }`,
      );
    }
...
function buildBlockFileMap(list: BlockMapFile[]): Map<string, BlockMapFile> {
