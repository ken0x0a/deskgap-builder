...
export async function createSelfSignedCert(publisher: string) {
  ...
  const tempPrefix = path.join(
    await tmpDir.getTempDir({ prefix: "self-signed-cert-creator" }),
    sanitizeFileName(publisher),
  );
  ...
  try {
    ...
    log.info({ file: pfx }, `created. Please see https://deskgap.build/code-signing how to use it to sign.`);

    const certLocation = "Cert:\\LocalMachine\\TrustedPeople";
    log.info(
      { file: pfx, certLocation },
