...
export default class SquirrelWindowsTarget extends Target {
  ...
  readonly options: SquirrelWindowsOptions = {
    ...
    ...this.packager.config.squirrelWindows,
  } as SquirrelWindowsOptions;
  ...
  async build(appOutDir: string, arch: Arch) {
    ...
    const setupFile = packager.expandArtifactNamePattern(
      this.options,
      "exe",
      arch,
    ...
    if (arch === Arch.ia32)
      log.warn(
        "For windows consider only distributing 64-bit or use nsis target, see https://github.com/deskgap-userland/deskgap-builder/issues/359#issuecomment-214851130",
      );
  ...
  async computeEffectiveDistOptions(): Promise<SquirrelOptions> {
    ...
    if (iconUrl == null) {
      ...
      if (iconUrl == null)
        throw new InvalidConfigurationError(
          "squirrelWindows.iconUrl is not specified, please see https://www.deskgap.build/configuration/squirrel-windows#SquirrelWindowsOptions-iconUrl",
        );
    }
    ...
    const options: SquirrelOptions = {
      ...
      packageCompressionLevel: parseInt(
        (process.env.ELECTRON_BUILDER_COMPRESSION_LEVEL || packager.compression === "store" ? 0 : 9) as any,
        10,
      ),
      vendorPath: await getBinFromUrl(
        "Squirrel.Windows",
        "1.9.0",
        "zJHk4CMATM7jHJ2ojRH1n3LkOnaIezDk5FAzJmlSEQSiEdRuB4GGLCegLDtsRCakfHIVfKh3ysJHLjynPkXwhQ==",
      ),
      ...(this.options as any),
    };
    ...
    if (options.remoteToken == null) options.remoteToken = process.env.GH_TOKEN || process.env.GITHUB_TOKEN;
    ...
    if (this.options.remoteReleases === true) {
      const info = await packager.info.repositoryInfo;
      if (info == null) log.warn("remoteReleases set to true, but cannot get repository info");
      else {
...
function checkConflictingOptions(options: any) {
  for (const name of [
    ...
    if (name in options) throw new InvalidConfigurationError(`Option ${name} is ignored, do not specify it.`);
