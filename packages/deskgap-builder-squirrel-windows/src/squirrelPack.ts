...
import { Arch, debug, exec, isEmptyOrSpaces, log, spawn } from "builder-util";
...
export function convertVersion(version: string): string {
  ...
  if (parts.length > 0) return [mainVersion, parts.join("-").replace(/\./g, "")].join("-");
...
function syncReleases(outputDirectory: string, options: SquirrelOptions) {
  ...
  const args = prepareArgs(
    ["-u", options.remoteReleases!, "-r", outputDirectory],
    path.join(options.vendorPath, "SyncReleases.exe"),
  );
  if (options.remoteToken) args.push("-t", options.remoteToken);
...
export class SquirrelBuilder {
  ...
  async buildInstaller(outFileNames: OutFileNames, appOutDir: string, outDir: string, arch: Arch) {
    ...
    await Promise.all([
      ...
      Promise.all([
        ...
        remove(path.join(outputDirectory, "RELEASES")),
      ]).then(() => ensureDir(outputDirectory)),
    ]);

    if (isEmptyOrSpaces(options.description)) options.description = options.productName;

    if (options.remoteReleases) await syncReleases(outputDirectory, options);
    ...
    await Promise.all<any>([
      ...
      copyFile(
        options.loadingGif
          ? path.resolve(packager.projectDir, options.loadingGif)
          : path.join(options.vendorPath, "install-spinner.gif"),
    ...
    await this.releasify(nupkgPath, outFileNames.packageFile).then((it) =>
      writeFile(path.join(dirToArchive, "RELEASES"), it),
    );
  ...
  private async releasify(nupkgPath: string, packageName: string) {
    ...
    if (debug.enabled) debug(`Squirrel output: ${out}`);

    const lines = out.split("\n");
    for (let i = lines.length - 1; i > -1; i--) {
      const line = lines[i];
      if (line.includes(packageName)) return line.trim();
    }
...
async function pack(
  ...
  const archive = archiver("zip", {
    zlib: { level: Math.max(1, options.packageCompressionLevel == null ? 9 : options.packageCompressionLevel) },
  });
...
function execSw(options: SquirrelOptions, args: string[]) {
  return exec(
    process.platform === "win32" ? path.join(options.vendorPath, "Update.com") : "mono",
    prepareArgs(args, path.join(options.vendorPath, "Update-Mono.exe")),
    {
      env: {
        ...process.env,
        SZA_PATH: path7za,
...
async function msi(
  ...
  // noinspection SpellCheckingInspection
  await exec(
    path.join(options.vendorPath, "candle.exe"),
    ["-nologo", "-ext", "WixNetFxExtension", "-out", "Setup.wixobj", "Setup.wxs"],
    {
      cwd: outputDirectory,
    },
  );
  // noinspection SpellCheckingInspection
  await exec(
    path.join(options.vendorPath, "light.exe"),
    ["-ext", "WixNetFxExtension", "-sval", "-out", outFile, "Setup.wixobj"],
    {
      cwd: outputDirectory,
    },
  );
