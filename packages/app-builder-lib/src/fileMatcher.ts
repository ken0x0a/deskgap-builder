...
import { copyDir, copyOrLinkFile, FileTransformer, Filter, statOrNull, USE_HARD_LINKS } from "builder-util/out/fs";
...
// https://github.com/deskgap-userland/deskgap-builder/issues/733
...
function ensureNoEndSlash(file: string): string {
  if (path.sep !== "/") file = file.replace(/\//g, path.sep);

  if (path.sep !== "\\") file = file.replace(/\\/g, path.sep);

  if (file.endsWith(path.sep)) return file.substring(0, file.length - 1);
...
export class FileMatcher {
  excludePatterns: Minimatch[] | null = null;
  ...
  computeParsedPatterns(result: Minimatch[], fromDir?: string): void {
    ...
    for (let pattern of this.patterns) {
      if (relativeFrom != null) pattern = path.join(relativeFrom, pattern);
      ...
      if (!pattern.includes(".") && !hasMagic(parsedPattern))
        // https://github.com/deskgap-userland/deskgap-builder/issues/545
        // add **/*
  ...
  normalizePattern(pattern: string) {
    if (pattern.startsWith("./")) pattern = pattern.substring("./".length);
...
export function getMainFileMatchers(
  ...
  if (matchers == null) matchers = [new FileMatcher(appDir, destination, macroExpander)];

  const matcher = matchers[0];
  // add default patterns, but only if from equals to app dir
  if (matcher.from !== appDir) return matchers;

  // https://github.com/deskgap-userland/deskgap-builder/issues/1741#issuecomment-311111418 so, do not use inclusive patterns
  const { patterns } = matcher;

  const customFirstPatterns: string[] = [];
  // deskgap-webpack - we need to copy only package.json and node_modules from root dir (and these files are added by default), so, explicit empty array is specified
  if (!matcher.isSpecifiedAsEmptyArray && (matcher.isEmpty() || matcher.containsOnlyIgnore()))
    customFirstPatterns.push("**/*");
  else if (!patterns.includes("package.json")) patterns.push("package.json");

  // https://github.com/deskgap-userland/deskgap-builder/issues/1482
  ...
  if (!relativeOutDir.startsWith(".")) customFirstPatterns.push(`!${relativeOutDir}{,/**/*}`);
  ...
  patterns.push("!**/._*");
  patterns.push("!**/deskgap-builder.{yaml,yml,json,json5,toml}");
  patterns.push(`!**/{${excludedNames}}`);

  if (isDeskGapCompile) patterns.push("!.cache{,/**/*}");

  patterns.push("!.yarn{,/**/*}");

  // https://github.com/deskgap-userland/deskgap-builder/issues/1969
...
export function getNodeModuleFileMatcher(
  ...
  // https://github.com/deskgap-userland/deskgap-builder/pull/2948#issuecomment-392241632
  // grab only excludes
  const matcher = new FileMatcher(appDir, destination, macroExpander);

  function addPatterns(patterns: (string | FileSet)[] | string | null | undefined | FileSet) {
    ...
    for (const pattern of patterns)
      if (typeof pattern === "string") {
        ...
        if (fileSet.from == null || fileSet.from === ".")
          for (const p of asArray(fileSet.filter)) matcher.addPattern(p);
      }
  }
...
export function getFileMatchers(
  ...
  const fileMatchers: FileMatcher[] = [];
  ...
  function addPatterns(patterns: (string | FileSet)[] | string | null | undefined | FileSet) {
    ...
    for (const pattern of patterns)
      ...
      else if (name === "asarUnpack") throw new Error(`Advanced file copying not supported for "${name}"`);
      else {
  ...
  if (name !== "extraDistFiles") addPatterns((config as any)[name]);
  ...
  if (!relativeOutDir.startsWith(".")) defaultMatcher.addPattern(`!${relativeOutDir}/*-unpacked{,/**/*}`);
...
export function copyFiles(
  matchers: FileMatcher[] | null,
  transformer: FileTransformer | null,
  isUseHardLink?: boolean,
): Promise<any> {
  if (matchers == null || matchers.length === 0) return Promise.resolve();
  ...
  return BluebirdPromise.map(matchers, async (matcher: FileMatcher) => {
    ...
    if (fromStat.isFile()) {
      ...
      // https://github.com/deskgap-userland/deskgap-builder/issues/1245
      if (toStat?.isDirectory())
        return await copyOrLinkFile(
          matcher.from,
          path.join(matcher.to, path.basename(matcher.from)),
          fromStat,
          isUseHardLink,
        );
    ...
    return await copyDir(matcher.from, matcher.to, {
      filter: matcher.createFilter(),
      transformer,
      isUseHardLink: isUseHardLink ? USE_HARD_LINKS : null,
    });
  });
}
