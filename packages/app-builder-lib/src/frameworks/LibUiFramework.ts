...
export class LibUiFramework implements Framework {
  ...
  async prepareApplicationStageDirectory(options: PrepareApplicationStageDirectoryOptions) {
    ...
    if (platform === Platform.MAC) await this.prepareMacosApplicationStageDirectory(packager as MacPackager, options);
    else if (platform === Platform.LINUX) await this.prepareLinuxApplicationStageDirectory(options);
  }
  ...
  private async prepareMacosApplicationStageDirectory(
    ...
    await executeAppBuilder([
      ...
      "darwin",
      "--stage",
      path.join(appContentsDir, "MacOS"),
    ]);
