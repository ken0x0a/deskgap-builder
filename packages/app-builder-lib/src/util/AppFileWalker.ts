import { FileConsumer, Filter } from "builder-util/out/fs";
...
function createAppFilter(matcher: FileMatcher, packager: Packager): Filter | null {
  if (packager.areNodeModulesHandledExternally) return matcher.isEmpty() ? null : matcher.createFilter();

  const nodeModulesFilter: Filter = (file, fileStat) => {
    return !(fileStat.isDirectory() && file.endsWith(nodeModulesSystemDependentSuffix));
  };

  if (matcher.isEmpty()) return nodeModulesFilter;

  const filter = matcher.createFilter();
  return (file, fileStat) => {
    if (!nodeModulesFilter(file, fileStat)) return false;
...
export class AppFileWalker extends FileCopyHelper implements FileConsumer {
  ...
  consume(file: string, fileStat: Stats, parent: string, siblingNames: string[]): any {
    if (fileStat.isDirectory()) {
      ...
      if (file.endsWith(nodeModulesSystemDependentSuffix)) return false;
    }
    // save memory - no need to store stat for directory
    else this.metadata.set(file, fileStat);
