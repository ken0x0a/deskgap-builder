...
function mergePublish(config: Configuration, configFromOptions: Configuration) {
  ...
  if (publish != null) delete (configFromOptions as any).publish;

  deepAssign(config, configFromOptions);

  if (publish == null) return;

  const listOnDisk = config.publish as any[];
  if (listOnDisk.length === 0) config.publish = publish;
  // apply to first
  else Object.assign(listOnDisk[0], publish);
}
...
export async function getConfig(
  ...
  if (configFromOptions != null) mergePublish(config, configFromOptions);
  ...
  for (let i = 0; i < value.length; i++) {
    ...
    if (typeof item === "string") {
      ...
      if (i !== 0) {
        ...
        do prevItem = value[prevItemIndex--] as FileSet;
        while (prevItem == null);

        if (prevItem.from == null && prevItem.to == null) {
          if (prevItem.filter == null) prevItem.filter = [item];
          else (prevItem.filter as string[]).push(item);
    ...
    if (item.to === ".") item.to = undefined;

    if (item.filter != null && typeof item.filter === "string") item.filter = [item.filter];
  }
...
function mergeFiles(
  ...
  const list = configuration[name] as FileSet[] | null;
  const parentList = parentConfiguration[name] as FileSet[] | null;
  if (list == null || parentList == null) return;

  const result = list.slice();
  mergedConfiguration[name] = result;

  itemLoop: for (const item of parentConfiguration.files as FileSet[]) {
      ...
      if (existingItem.from === item.from && existingItem.to === item.to) {
        if (item.filter != null)
          if (existingItem.filter == null) {
            ...
            existingItem.filter = (item.filter as string[]).concat(existingItem.filter);
          }
...
export function doMergeConfigs(configuration: Configuration, parentConfiguration: Configuration | null) {
  ...
  if (parentConfiguration == null) return deepAssign(getDefaultConfig(), configuration);
...
export async function validateConfig(config: Configuration, debugLogger: DebugLogger) {
  ...
  if (oldConfig.npmSkipBuildFromSource === false)
    throw new InvalidConfigurationError(
      `npmSkipBuildFromSource is deprecated, please use buildDependenciesFromSource"`,
    );

  if (oldConfig.appImage?.systemIntegration != null)
    throw new InvalidConfigurationError(
  ...
  validateSchema(await schemeDataPromise.value, config, {
    ...
    postFormatter: (formattedError: string, error: any): string => {
      if (debugLogger.isEnabled) debugLogger.add("invalidConfig", safeStringifyJson(error));
      ...
      if (targetPath != null && targets.has(targetPath)) url = `${site}/configuration/${targetPath}`;
...
export async function computeDefaultAppDirectory(
  ...
  if (userAppDir != null) {
    ...
    if (stat == null) throw new InvalidConfigurationError(`Application directory ${userAppDir} doesn't exist`);
