...
export class BuildCacheManager {
  ...
  async copyIfValid(digest: string): Promise<boolean> {
    ...
    try {
      ...
      if (e.code === "ENOENT" || e.code === "ENOTDIR") log.debug({ error: e.code }, "copy cached executable failed");
      else log.warn({ error: e.stack || e }, `cannot copy cached executable`);
    }
    return false;
  }

  async save() {
    if (this.newDigest == null) throw new Error("call copyIfValid before");

    if (this.cacheInfo == null) this.cacheInfo = { executableDigest: this.newDigest };
    else this.cacheInfo.executableDigest = this.newDigest;

    try {
      await ensureDir(this.cacheDir);
      await Promise.all([
        writeJson(this.cacheInfoFile, this.cacheInfo),
        copyFile(this.executableFile, this.cacheFile, false),
      ]);
    } catch (e) {
...
export async function digest(hash: Hash, files: string[]) {
  // do not use pipe - better do bulk file read (https://github.com/yarnpkg/yarn/commit/7a63e0d23c46a4564bc06645caf8a59690f04d01)
  for (const content of await BluebirdPromise.map(files, (it) => readFile(it))) hash.update(content);
