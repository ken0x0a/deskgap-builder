...
export function getRepositoryInfo(
  ...
  return _getInfo(
    projectDir,
    (devMetadata == null ? null : devMetadata.repository) || (metadata == null ? null : metadata.repository),
  );
}
...
async function getGitUrlFromGitConfig(projectDir: string): Promise<string | null> {
  ...
  if (i !== -1) {
    let u = conf[i + 1];
    if (!u.match(/^\s*url =/)) u = conf[i + 2];

    if (u.match(/^\s*url =/)) return u.replace(/^\s*url = /, "");
  }
  return null;
}
...
async function _getInfo(
  ...
  if (repo != null) return parseRepositoryUrl(typeof repo === "string" ? repo : repo.url);
