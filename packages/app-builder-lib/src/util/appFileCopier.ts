...
import { createDeskGapCompilerHost, NODE_MODULES_PATTERN } from "../fileTransformer";
...
export function getDestinationPath(file: string, fileSet: ResolvedFileSet) {
  ...
  if (index < 0 && file.endsWith(`${path.sep}node_modules`)) index = file.length - 13;

  if (index < 0) throw new Error(`File "${file}" not under the source directory "${fileSet.src}"`);
...
export async function copyAppFiles(fileSet: ResolvedFileSet, packager: Packager, transformer: FileTransformer) {
  ...
  const fileCopier = new FileCopier((file) => {
    // https://github.com/deskgap-userland/deskgap-builder/issues/3038
  ...
  for (let i = 0, n = fileSet.files.length; i < n; i++) {
    ...
    if (taskManager.tasks.length > MAX_FILE_REQUESTS) await taskManager.awaitTasks();
  }

  if (taskManager.tasks.length > 0) await taskManager.awaitTasks();

  if (links.length > 0) await BluebirdPromise.map(links, (it) => symlink(it.link, it.file), CONCURRENCY);
}
...
export async function computeFileSets(
...
): Promise<ResolvedFileSet[]> {
  const fileSets: ResolvedFileSet[] = [];
  ...
  for (const matcher of matchers) {
    ...
    const { metadata } = fileWalker;
    fileSets.push(validateFileSet({ src: matcher.from, files, metadata, destination: matcher.to }));
  }

  if (isDeskGapCompile)
    // cache files should be first (better IO)
    fileSets.unshift(await compileUsingDeskGapCompile(fileSets[0], packager));

  return fileSets;
}

function getNodeModuleExcludedExts(platformPackager: PlatformPackager<any>) {
  // do not exclude *.h files (https://github.com/deskgap-userland/deskgap-builder/issues/2852)
  const result = [".o", ".obj"].concat(excludedExts.split(",").map((it) => `.${it}`));
  if (platformPackager.config.includePdb !== true) result.push(".pdb");
...
function validateFileSet(fileSet: ResolvedFileSet): ResolvedFileSet {
  if (fileSet.src == null || fileSet.src.length === 0) throw new Error("fileset src is empty");
...
async function compileUsingDeskGapCompile(mainFileSet: ResolvedFileSet, packager: Packager): Promise<ResolvedFileSet> {
  log.info("compiling using deskgap-compile");

  const deskgapCompileCache = await packager.tempDirManager.getTempDir({ prefix: "deskgap-compile-cache" });
  const cacheDir = path.join(deskgapCompileCache, ".cache");
  // clear and create cache dir
  await ensureDir(cacheDir);
  const compilerHost = await createDeskGapCompilerHost(mainFileSet.src, cacheDir);
  const nextSlashIndex = mainFileSet.src.length + 1;
  // pre-compute deskgap-compile to cache dir - we need to process only subdirectories, not direct files of app dir
  ...
  const cacheFiles = await walk(cacheDir, (file) => !file.startsWith("."), {
    consume: (file, fileStat) => {
      if (fileStat.isFile()) metadata.set(file, fileStat);
  ...
  if (mainFileSet.transformedFiles == null) mainFileSet.transformedFiles = new Map();

  mainFileSet.transformedFiles.set(
    mainFileSet.files.length - 1,
    `
'use strict';
require('deskgap-compile').init(__dirname, require('path').resolve(__dirname, '${
      packager.metadata.main || "index"
    }'), true);
`,
  );
  return { src: deskgapCompileCache, files: cacheFiles, metadata, destination: mainFileSet.destination };
}
