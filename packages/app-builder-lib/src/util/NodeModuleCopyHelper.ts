...
export class NodeModuleCopyHelper extends FileCopyHelper {
  ...
  async collectNodeModules(
    baseDir: string,
    moduleNames: Iterable<string>,
    nodeModuleExcludedExts: string[],
  ): Promise<string[]> {
    const { filter } = this;
    const { metadata } = this;
    ...
    for (const moduleName of moduleNames) {
      ...
      while (queue.length > 0) {
        ...
        const sortedFilePaths = await BluebirdPromise.map(
          childNames,
          (name) => {
            if (onNodeModuleFile != null) onNodeModuleFile(dirPath + path.sep + name);

            if (excludedFiles.has(name) || name.startsWith("._")) return null;

            for (const ext of nodeModuleExcludedExts) if (name.endsWith(ext)) return null;

            // noinspection SpellCheckingInspection
            if (
              isTopLevel &&
              (topLevelExcludedFiles.has(name) ||
                (moduleName === "libui-node" && (name === "build" || name === "docs" || name === "src")))
            )
              return null;
            ...
            if (dirPath.endsWith("build")) {
              if (
                ...
                return null;
            } else if (dirPath.endsWith("Release") && (name === ".deps" || name === "obj.target")) return null;
            else if (name === "src" && (dirPath.endsWith("keytar") || dirPath.endsWith("keytar-prebuild"))) return null;
            else if (dirPath.endsWith("lzma-native") && (name === "build" || name === "deps")) return null;

            const filePath = dirPath + path.sep + name;
            return lstat(filePath).then((stat) => {
              if (filter != null && !filter(filePath, stat)) return null;

              if (!stat.isDirectory()) metadata.set(filePath, stat);
