...
async function getReleaseInfo(packager: PlatformPackager<any>) {
  ...
  if (releaseInfo.releaseNotes == null) {
    ...
    if (releaseNotes != null) releaseInfo.releaseNotes = releaseNotes;
  }
 ...
 */
function computeChannelNames(packager: PlatformPackager<any>, publishConfig: PublishConfiguration): string[] {
  ...
  if (
    ...
    !isGenerateUpdatesFilesForAllChannels(packager)
  )
...
function getArchPrefixForUpdateFile(arch: Arch | null, packager: PlatformPackager<any>) {
  if (arch == null || arch === Arch.x64 || packager.platform !== Platform.LINUX) return "";
...
function computeIsisDeskGapUpdater1xCompatibility(
  updaterCompatibility: string | null,
  publishConfiguration: PublishConfiguration,
  packager: Packager,
) {
  if (updaterCompatibility != null) return semver.satisfies("1.0.0", updaterCompatibility);

  // spaces is a new publish provider, no need to keep backward compatibility
  if (publishConfiguration.provider === "spaces") return false;

  const updaterVersion =
    packager.metadata.dependencies == null ? null : packager.metadata.dependencies["deskgap-updater"];
  return updaterVersion == null || semver.lt(updaterVersion, "4.0.0");
}

/** @internal */
export async function createUpdateInfoTasks(
  event: ArtifactCreated,
  _publishConfigs: PublishConfiguration[],
): Promise<UpdateInfoFileTask[]> {
  const { packager } = event;
  const publishConfigs = await getPublishConfigsForUpdateInfo(packager, _publishConfigs, event.arch);
  if (publishConfigs == null || publishConfigs.length === 0) return [];

  const { outDir } = event.target!;
  ...
  const deskgapUpdaterCompatibility =
    packager.platformSpecificBuildOptions.deskgapUpdaterCompatibility ||
    packager.config.deskgapUpdaterCompatibility ||
    ">=2.15";
  for (const publishConfiguration of publishConfigs) {
    ...
    let isDeskGapUpdater1xCompatibility = computeIsisDeskGapUpdater1xCompatibility(
      deskgapUpdaterCompatibility,
    ...
    for (const channel of computeChannelNames(packager, publishConfiguration)) {
      ...
      const updateInfoFile = path.join(
        dir,
        (isBintray ? `${version}_` : "") + getUpdateInfoFileName(channel, packager, event.arch),
      );
      if (createdFiles.has(updateInfoFile)) continue;
...
async function createUpdateInfo(
  version: string,
  event: ArtifactCreated,
...
export async function writeUpdateInfoFiles(updateInfoFileTasks: UpdateInfoFileTask[], packager: Packager) {
  // zip must be first and zip info must be used for old path/sha512 properties in the update info
  updateInfoFileTasks.sort(
    (a, b) => (a.info.files[0].url.endsWith(".zip") ? 0 : 100) - (b.info.files[0].url.endsWith(".zip") ? 0 : 100),
  );
  ...
  await BluebirdPromise.map(
    ...
    async (task) => {
      ...
      if (task.info.releaseDate == null) task.info.releaseDate = releaseDate;
...
async function writeOldMacInfo(
  ...
  const updateInfoFile =
    isGitHub && outDir === dir
      ? path.join(dir, "github", `${channel}-mac.json`)
      : path.join(dir, `${channel}-mac.json`);
