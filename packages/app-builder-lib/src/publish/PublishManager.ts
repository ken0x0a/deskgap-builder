...
export class PublishManager implements PublishContext {
  ...
  private readonly updateFileWriteTask: UpdateInfoFileTask[] = [];
  ...
  constructor(
    ...
    if (!isPullRequest() || forcePublishForPr) {
      ...
      this.isPublish =
        publishPolicy != null &&
        publishOptions.publish !== "never" &&
        (publishPolicy !== "onTag" || getCiTag() != null);
      if (this.isPublish && forcePublishForPr) log.warn(publishForPrWarning);
    ...
    packager.artifactCreated((event) => {
      ...
      if (publishConfiguration == null)
        ...
        if (debug.enabled)
          debug(
            `artifactCreated (isPublish: ${this.isPublish}): ${safeStringifyJson(
              event,
              new Set(["packager"]),
  ...
  async awaitTasks(): Promise<void> {
    ...
    if (this.cancellationToken.cancelled || updateInfoFileTasks.length === 0) return;
  ...
  async getGlobalPublishConfigurations(): Promise<PublishConfiguration[] | null> {
  ...
  scheduleUpload(publishConfig: PublishConfiguration, event: UploadTask, appInfo: AppInfo): void {
    if (publishConfig.provider === "generic") return;
    ...
    const { providerName } = publisher;
    if (
      this.publishOptions.publish === "onTagOrDraft" &&
      getCiTag() == null &&
      !(providerName === "GitHub" || providerName === "Bintray")
    ) {
      log.info(
        { file: event.file, reason: "current build is not for a git tag", publishPolicy: "onTagOrDraft" },
  ...
  private async artifactCreatedWithoutExplicitPublishConfig(event: ArtifactCreated) {
    ...
    const publishConfigs = await getPublishConfigs(
      platformPackager,
      target == null ? null : target.options,
      event.arch,
      this.isPublish,
    );
    ...
    if (publishConfigs == null) {
      if (this.isPublish) log.debug({ file: eventFile, reason: "no publish configs" }, "not published");
    ...
    if (
      ...
      this.taskManager.addTask(
        createUpdateInfoTasks(event, publishConfigs).then((it) => this.updateFileWriteTask.push(...it)),
      );
  }
...
export async function getAppUpdatePublishConfiguration(
  ...
  const publishConfigs = await getPublishConfigsForUpdateInfo(
    packager,
    await getPublishConfigs(packager, null, arch, errorIfCannot),
    arch,
  );
  if (publishConfigs == null || publishConfigs.length === 0) return null;
  ...
  if (packager.platform === Platform.WINDOWS && publishConfig.publisherName == null) {
    ...
    const publisherName = winPackager.isForceCodeSigningVerification
      ? await winPackager.computedPublisherName.value
      : undefined;
...
export async function getPublishConfigsForUpdateInfo(
  packager: PlatformPackager<any>,
  publishConfigs: PublishConfiguration[] | null,
  arch: Arch | null,
): Promise<PublishConfiguration[] | null> {
  if (publishConfigs === null) return null;
...
export function createPublisher(
  ...
  if (debug.enabled) debug(`Create publisher: ${safeStringifyJson(publishConfig)}`);

  const { provider } = publishConfig;
  switch (provider) {
    case "github":
...
function requireProviderClass(provider: string, packager: Packager): any | null {
  switch (provider) {
    ...
    default: {
      ...
      if (module == null) module = require(name);
...
export function computeDownloadUrl(
  ...
  if (publishConfiguration.provider === "generic") {
    ...
    if (fileName == null) return baseUrlString;

    const baseUrl = url.parse(baseUrlString);
    return url.format({
      ...(baseUrl as url.UrlObject),
  ...
  if (publishConfiguration.provider === "github") {
    ...
    baseUrl = `${githubUrl(gh)}/${gh.owner}/${gh.repo}/releases/download/${gh.vPrefixedTagName === false ? "" : "v"}${
      packager.appInfo.version
    }`;
  } else baseUrl = getS3LikeProviderBaseUrl(publishConfiguration);

  if (fileName == null) return baseUrl;
...
async function resolvePublishConfigurations(
  ...
  if (publishers == null) {
    ...
    if (!isEmptyOrSpaces(process.env.GH_TOKEN) || !isEmptyOrSpaces(process.env.GITHUB_TOKEN)) serviceName = "github";
    else if (!isEmptyOrSpaces(process.env.BT_TOKEN)) serviceName = "bintray";

    if (serviceName != null) {
      log.debug(null, `detect ${serviceName} as publish provider`);
      return [
        (await getResolvedPublishConfig(platformPackager, packager, { provider: serviceName }, arch, errorIfCannot))!,
      ];
    }
  }
  ...
  return await (BluebirdPromise.map(asArray(publishers), (it) =>
    getResolvedPublishConfig(
      platformPackager,
      packager,
      typeof it === "string" ? { provider: it } : it,
      arch,
      errorIfCannot,
    ),
  ) as Promise<PublishConfiguration[]>);
}

function isSuitableWindowsTarget(target: Target) {
  if (target.name === "appx" && target.options != null && (target.options as any).deskgapUpdaterAware) return true;

  return target.name === "nsis" || target.name.startsWith("nsis-");
}

function expandPublishConfig(
  options: any,
  platformPackager: PlatformPackager<any> | null,
  packager: Packager,
  arch: Arch | null,
  ...
  for (const name of Object.keys(options)) {
    ...
    if (typeof value === "string") {
      ...
      const expanded =
        platformPackager == null
          ? expandMacro(value, archValue, packager.appInfo)
          : platformPackager.expandMacro(value, archValue);
      if (expanded !== value) options[name] = expanded;
    }
  }
}

function isDetectUpdateChannel(
  platformSpecificConfiguration: PlatformSpecificBuildOptions | null,
...
async function getResolvedPublishConfig(
  ...
  if (
    ...
    isDetectUpdateChannel(
      platformPackager == null ? null : platformPackager.platformSpecificBuildOptions,
      packager.config,
    )
  )
  ...
  if (provider === "generic") {
    ...
    if (o.url == null) throw new InvalidConfigurationError(`Please specify "url" for "generic" update server`);

    if (channelFromAppVersion != null) (o as any).channel = channelFromAppVersion;

    return options;
  }

  const providerClass = requireProviderClass(options.provider, packager);
  if (providerClass?.checkAndResolveOptions != null) {
    await providerClass.checkAndResolveOptions(options, channelFromAppVersion, errorIfCannot);
    return options;
  }

  const isGithub = provider === "github";
  if (!isGithub && provider !== "bintray") return options;
  ...
  async function getInfo() {
    ...
    if (errorIfCannot) throw new Error(message);
    else {
      log.warn(message);
      return null;
    }
  ...
  if (!owner || !project) {
    ...
    if (!owner) owner = info.user;

    if (!project) project = info.project;
  }

  if (isGithub) {
    if ((options as GithubOptions).token != null && !(options as GithubOptions).private)
      log.warn(
        '"token" specified in the github publish options. It should be used only for [setFeedURL](module:deskgap-updater/out/AppUpdater.AppUpdater+setFeedURL).',
      );
