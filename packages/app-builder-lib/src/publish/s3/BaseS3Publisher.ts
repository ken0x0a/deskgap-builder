...
export abstract class BaseS3Publisher extends Publisher {
  protected constructor(context: PublishContext, private readonly options: BaseS3Options) {
    super(context);
  }
  ...
  protected configureS3Options(args: string[]) {
    // if explicitly set to null, do not add
    if (this.options.acl !== null) args.push("--acl", this.options.acl || "public-read");
  }
