import { asArray, InvalidConfigurationError, log } from "builder-util/out/util";
...
export interface CustomWindowsSignTaskConfiguration extends WindowsSignTaskConfiguration {
  computeSignToolArgs(isWin: boolean): string[];
}
...
export async function sign(options: WindowsSignOptions, packager: WinPackager) {
  ...
  if (options.path.endsWith(".msi")) hashes = [hashes != null && !hashes.includes("sha1") ? "sha256" : "sha1"];
  else if (options.path.endsWith(".appx")) hashes = ["sha256"];
  else if (hashes == null) hashes = ["sha1", "sha256"];
  else hashes = Array.isArray(hashes) ? hashes : [hashes];
  ...
  for (const hash of hashes) {
    ...
    isNest = true;
    if (taskConfiguration.resultOutputPath != null) await rename(taskConfiguration.resultOutputPath, options.path);
  }
}
...
export async function getCertInfo(file: string, password: string): Promise<CertificateInfo> {
  ...
  const errorMessagePrefix =
    "Cannot extract publisher name from code signing certificate. As workaround, set win.publisherName. Error: ";
  try {
...
export async function getCertificateFromStoreInfo(
  ...
  const { certificateSubjectName } = options;
...
export async function doSign(configuration: CustomWindowsSignTaskConfiguration, packager: WinPackager) {
  ...
  if (
    ...
    !("file" in configuration.cscInfo!) /* certificateSubjectName and other such options */
  ) {
    ...
    if (toolInfo.env != null) env = toolInfo.env;
  }
...
function computeSignToolArgs(
  options: WindowsSignTaskConfiguration,
  isWin: boolean,
  vm: VmManager = new VmManager(),
): string[] {
  const inputFile = vm.toVmFile(options.path);
  const outputPath = isWin ? inputFile : getOutputPath(inputFile, options.hash);
  if (!isWin) options.resultOutputPath = outputPath;
  ...
  if (process.env.ELECTRON_BUILDER_OFFLINE !== "true") {
    ...
    else args.push("-t", timestampingServiceUrl);
  }
  ...
  if (certificateFile == null) {
    ...
    if (!isWin)
      throw new Error(
        `${subjectName == null ? "certificateSha1" : "certificateSubjectName"} supported only on Windows`,
      );

    args.push("/sha1", cscInfo.thumbprint);
    args.push("/s", cscInfo.store);
    if (cscInfo.isLocalMachineStore) args.push("/sm");
  } else {
    const certExtension = path.extname(certificateFile);
    if (certExtension === ".p12" || certExtension === ".pfx")
      args.push(isWin ? "/f" : "-pkcs12", vm.toVmFile(certificateFile));
    else throw new Error(`Please specify pkcs12 (.p12/.pfx) file, ${certificateFile} is not correct`);
  }

  if (!isWin || options.hash !== "sha1") {
    args.push(isWin ? "/fd" : "-h", options.hash);
    if (isWin && process.env.ELECTRON_BUILDER_OFFLINE !== "true") args.push("/td", "sha256");
  }

  if (options.name) args.push(isWin ? "/d" : "-n", options.name);

  if (options.site) args.push(isWin ? "/du" : "-i", options.site);

  // msi does not support dual-signing
  if (options.isNest) args.push(isWin ? "/as" : "-nest");

  const password = options.cscInfo == null ? null : (options.cscInfo as FileCodeSigningInfo).password;
  if (password) args.push(isWin ? "/p" : "-pass", password);
  ...
  if (!isWin && httpsProxyFromEnv != null && httpsProxyFromEnv.length) args.push("-p", httpsProxyFromEnv);
...
function getWinSignTool(vendorPath: string): string {
  ...
  if (isOldWin6()) return path.join(vendorPath, "windows-6", "signtool.exe");

  return path.join(vendorPath, "windows-10", process.arch, "signtool.exe");
}

async function getToolPath(): Promise<ToolInfo> {
  if (isUseSystemSigncode()) return { path: "osslsigncode" };
