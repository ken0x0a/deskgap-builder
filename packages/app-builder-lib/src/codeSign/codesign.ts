...
export async function downloadCertificate(urlOrBase64: string, tmpDir: TmpDir, currentDir: string): Promise<string> {
  ...
  if ((urlOrBase64.length > 3 && urlOrBase64[1] === ":") || urlOrBase64.startsWith("/") || urlOrBase64.startsWith("."))
    file = urlOrBase64;
  else if (urlOrBase64.startsWith("file://")) file = urlOrBase64.substring("file://".length);
  else if (urlOrBase64.startsWith("~/")) file = path.join(homedir(), urlOrBase64.substring("~/".length));
  else {
    ...
    if (isUrl || urlOrBase64.length > 2048 || urlOrBase64.endsWith("=")) {
      ...
      if (isUrl) await download(urlOrBase64, tempFile);
      else await outputFile(tempFile, Buffer.from(urlOrBase64, "base64"));
  ...
  if (stat == null) throw new InvalidConfigurationError(`${file} doesn't exist`);
  else if (!stat.isFile()) throw new InvalidConfigurationError(`${file} not a file`);
  else return file;
}
