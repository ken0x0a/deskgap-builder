...
export function isSignAllowed(isPrintWarn = true): boolean {
  if (process.platform !== "darwin") {
    if (isPrintWarn) log.warn({ reason: "supported only on macOS" }, "skipped macOS application code signing");
  ...
  if (isPullRequest())
    if (isEnvTrue(process.env.CSC_FOR_PULL_REQUEST)) {
      if (isPrintWarn) log.warn(buildForPrWarning);
    } else {
      if (isPrintWarn)
...
export async function reportError(
  ...
  if (qualifier == null) {
    ...
    if (isAutoDiscoveryCodeSignIdentity())
      logFields.reason += `cannot find valid "${certificateType}" identity${
        isMas ? "" : ` or custom non-Apple code signing certificate`
      }`;

    logFields.reason += ", see https://deskgap.build/code-signing";
    if (!isAutoDiscoveryCodeSignIdentity()) logFields.CSC_IDENTITY_AUTO_DISCOVERY = false;
  } else {
  ...
  if (keychainFile != null) args.push(keychainFile);
  ...
  else log.warn(logFields, "skipped macOS application code signing");
}
...
const bundledCertKeychainAdded = new Lazy<void>(async () => {
  ...
  const results = await Promise.all<any>([
    listUserKeychains(),
    copyFile(path.join(__dirname, "..", "..", "certs", "root_certs.keychain"), tmpKeychainPath).then(() =>
      rename(tmpKeychainPath, keychainPath),
    ),
  ]);
...
function listUserKeychains(): Promise<string[]> {
...
export function removeKeychain(keychainFile: string, printWarn = true): Promise<any> {
  return exec("security", ["delete-keychain", keychainFile]).catch((e) => {
    if (printWarn) log.warn({ file: keychainFile, error: e.stack || e }, "cannot delete keychain");
...
export async function createKeychain({
  ...
  cscIKeyPassword,
  currentDir,
}: CreateKeychainOptions): Promise<CodeSigningInfo> {
  // travis has correct AppleWWDRCA cert
  if (process.env.TRAVIS !== "true") await bundledCertKeychainAdded.value;
  ...
  if (cscILink != null) certLinks.push(cscILink);
  ...
  await Promise.all([
    ...
    BluebirdPromise.map(certLinks, (link, i) =>
      downloadCertificate(link, tmpDir, currentDir).then((it) => (certPaths[i] = it)),
    ),
    BluebirdPromise.mapSeries(securityCommands, (it) => exec("security", it)),
  ]);
  return await importCerts(
    keychainFile,
    certPaths,
    [cscKeyPassword, cscIKeyPassword].filter((it) => it != null) as string[],
  );
}

async function importCerts(keychainFile: string, paths: string[], keyPasswords: string[]): Promise<CodeSigningInfo> {
...
export function sign(path: string, name: string, keychain: string): Promise<any> {
  ...
  if (keychain != null) args.push("--keychain", keychain);
...
async function getValidIdentities(keychain?: string | null): Promise<string[]> {
  function addKeychain(args: string[]) {
    if (keychain != null) args.push(keychain);
  ...
  if (result == null || keychain != null) {
    ...
    result = Promise.all<string[]>([
      exec("security", addKeychain(["find-identity", "-v"])).then((it) =>
        it
          .trim()
          .split("\n")
          .filter((it) => {
            for (const prefix of appleCertificatePrefixes) if (it.includes(prefix)) return true;
      ...
      const array = it[0]
        ...
        .filter(
          (it) =>
            ...
            !it.includes("valid identities found") &&
            !it.includes("iPhone ") &&
    ...
    if (keychain == null) findIdentityRawResult = result;
  }
  return result;
}

async function _findIdentity(
  type: CertType,
  ...
  for (const line of lines) {
    ...
    if (line.includes(namePrefix)) return parseIdentity(line);
  }
  ...
  if (type === "Developer ID Application")
    ...
    l: for (const line of lines) {
      ...
      if (line.includes("Mac Developer:")) continue;

      for (const prefix of appleCertificatePrefixes) if (line.includes(prefix)) continue l;
...
export function findIdentity(
  ...
  for (const prefix of appleCertificatePrefixes) checkPrefix(identity, prefix);
...
function checkPrefix(name: string, prefix: string) {
  if (name.startsWith(prefix))
    throw new InvalidConfigurationError(
      `Please remove prefix "${prefix}" from the specified name — appropriate certificate will be chosen automatically`,
    );
}
