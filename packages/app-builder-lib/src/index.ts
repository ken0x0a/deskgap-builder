...
import { PublishOptions } from "deskgap-publish/out/publisher";
...
export { DeskGapDownloadOptions, DeskGapPlatformName } from "./deskgap/DeskGapFramework";
export {
  ...
  Protocol,
  ReleaseInfo,
} from "./options/PlatformSpecificBuildOptions";
...
export { PublishOptions, UploadTask } from "deskgap-publish";
...
export function build(
  ...
  packager: Packager = new Packager(options),
): Promise<string[]> {
  ...
  const promise = packager.build().then(async (buildResult) => {
    const afterAllArtifactBuild = resolveFunction(
    ...
    if (afterAllArtifactBuild != null) {
      ...
      if (newArtifacts.length === 0 || !publishManager.isPublish) return buildResult.artifactPaths;

      const publishConfigurations = await publishManager.getGlobalPublishConfigurations();
      if (publishConfigurations == null || publishConfigurations.length === 0) return buildResult.artifactPaths;
  ...
  return executeFinally(promise, (isErrorOccurred) => {
    ...
    } else promise = publishManager.awaitTasks();
