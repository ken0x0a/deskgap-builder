...
export async function getDeskGapVersion(
  ...
  projectMetadata: MetadataValue = new Lazy(() =>
    orNullIfFileNotExist(readJson(path.join(projectDir, "package.json"))),
  ),
): Promise<string> {
  if (config == null) config = await getConfig(projectDir, null, null);

  if (config.deskgapVersion != null) return config.deskgapVersion;
  ...
  for (const name of deskgapPackages)
    try {
      ...
      try {
        ...
        if (e.code !== "ENOENT") log.warn({ name, error: e }, `cannot read deskgap version package.json`);
      }
    }
...
export async function getDeskGapPackage(projectDir: string) {
  for (const name of deskgapPackages)
    try {
      ...
      if (e.code !== "ENOENT") log.warn({ name, error: e }, `cannot find deskgap in package.json`);
    }
...
function findFromPackageMetadata(packageData: any): NameAndVersion | null {
  for (const name of deskgapPackages) {
    ...
    if (dep != null) return { name, version: dep };
  }
  return null;
}
