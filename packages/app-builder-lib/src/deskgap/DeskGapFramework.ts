...
async function beforeCopyExtraFiles(options: BeforeCopyExtraFilesOptions) {
  ...
  if (packager.platform === Platform.LINUX) {
    ...
    if (wantedLanguages.length === 0) return;
    ...
    await BluebirdPromise.map(
      ...
      (file) => {
        if (!file.endsWith(langFileExt)) return;

        const language = file.substring(0, file.length - langFileExt.length);
        if (!wantedLanguages.includes(language)) return remove(path.join(resourcesDir, file));
...
class DeskGapFramework implements Framework {
  ...
  getDefaultIcon(platform: Platform) {
    if (platform === Platform.LINUX) return path.join(getTemplatePath("icons"), "deskgap-linux");
...
export async function createDeskGapFrameworkSupport(
  ...
  if (version == null) {
    ...
    if (packager.isPrepackedAppAsar) {
      ...
      if (version == null) throw new Error(`Cannot compute deskgap version for prepacked asar`);
    } else
...
async function unpack(
  ...
  log.debug({ dist }, "dist");
  if (dist == null) {
    if (isSafeToUnpackDeskGapOnRemoteBuildServer(packager)) return;
