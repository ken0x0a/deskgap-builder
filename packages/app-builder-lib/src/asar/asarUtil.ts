...
export class AsarPackager {
  ...
  async pack(fileSets: ResolvedFileSet[], packager: PlatformPackager<any>) {
  ...
  private async createPackageFromFiles(fileSet: ResolvedFileSet, packager: Packager) {
    ...
    const correctDirNodeUnpackedFlag = async (filePathInArchive: string, dirNode: Node) => {
      for (const dir of unpackedDirs)
        if (
          filePathInArchive.length > dir.length + 2 &&
          filePathInArchive[dir.length] === path.sep &&
          filePathInArchive.startsWith(dir)
        ) {
    ...
    const { transformedFiles } = fileSet;
    ...
    for (let i = 0, n = fileSet.files.length; i < n; i++) {
      ...
      const isUnpacked = (dirNode.unpacked || this.unpackPattern?.(file, stat)) ?? false;
  ...
  private writeAsarFile(
    fileSets: ResolvedFileSet[],
    unpackedFileIndexMap: Map<ResolvedFileSet, Set<number>>,
  ): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      let { files } = fileSets[0];
      let { metadata } = fileSets[0];
      let { transformedFiles } = fileSets[0];
      ...
      const w = (index: number) => {
        while (true) {
          ...
          if (!unpackedFileIndexSet.has(index)) break;
          else {
            const stat = metadata.get(files[index]);
            if (stat?.isSymbolicLink()) {
              symlink((stat as any).linkRelativeToFile, path.join(this.unpackedDest, (stat as any).pathInArchive), () =>
                w(index + 1),
              );
              return;
            }
...
async function order(filenames: string[], orderingFile: string, src: string) {
  ...
  for (const file of orderingFiles) {
    ...
    for (const pathComponent of pathComponents) ordering.push(path.join(src, pathComponent));
  }

  const sortedFiles: string[] = [];
  let missing = 0;
  const total = filenames.length;
  for (const file of ordering) if (!sortedFiles.includes(file) && filenames.includes(file)) sortedFiles.push(file);
...
function copyFileOrData(
  ...
  if (data == null) return fileCopier.copy(source, destination, stats);
