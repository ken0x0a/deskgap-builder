...
export class AsarFilesystem {
  ...
  constructor(readonly src: string, readonly header = new Node(), readonly headerSize: number = -1) {
    if (this.header.files == null) this.header.files = {};
  }

  addFileNode(file: string, dirNode: Node, size: number, unpacked: boolean, stat: Stats): Node {
    if (size > 4294967295) throw new Error(`${file}: file size cannot be larger than 4.2GB`);
    ...
    if (unpacked) node.unpacked = true;
      ...
      if (process.platform !== "win32" && stat.mode & 0o100) node.executable = true;
  ...
  getOrCreateNode(p: string): Node {
    if (p == null || p.length === 0) return this.header;

    const name = path.basename(p);
    const dirNode = this.searchNodeFromDirectory(path.dirname(p), true)!;
    if (dirNode.files == null) dirNode.files = {};
...
export async function readAsar(archive: string): Promise<AsarFilesystem> {
  ...
  try {
    ...
    if ((await read(fd, sizeBuf, 0, 8, null as any)).bytesRead !== 8) throw new Error("Unable to read header size");

    const sizePickle = createFromBuffer(sizeBuf);
    size = sizePickle.createIterator().readUInt32();
    headerBuf = Buffer.allocUnsafe(size);
    if ((await read(fd, headerBuf, 0, size, null as any)).bytesRead !== size) throw new Error("Unable to read header");
  } finally {
    await close(fd);
  }
...
async function readFileFromAsar(filesystem: AsarFilesystem, filename: string, info: Node): Promise<Buffer> {
  ...
  if (info.unpacked) return await readFile(path.join(`${filesystem.src}.unpacked`, filename));
