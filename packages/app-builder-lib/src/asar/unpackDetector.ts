...
/** @internal */
export async function detectUnpackedDirs(
  fileSet: ResolvedFileSet,
  autoUnpackDirs: Set<string>,
  unpackedDest: string,
  rootForAppFilesWithoutAsar: string,
) {
  const dirToCreate = new Map<string, string[]>();
  const { metadata } = fileSet;

  function addParents(child: string, root: string) {
    child = path.dirname(child);
    if (autoUnpackDirs.has(child)) return;
    ...
    do {
      ...
      if (child === root || p === root || autoUnpackDirs.has(p)) break;
  ...
  for (let i = 0, n = fileSet.files.length; i < n; i++) {
    ...
    if (nextSlashIndex < 0) continue;

    if (file[index + NODE_MODULES_PATTERN.length] === "@") nextSlashIndex = file.indexOf(path.sep, nextSlashIndex + 1);

    if (!metadata.get(file)!.isFile()) continue;
    ...
    // https://github.com/deskgap-userland/deskgap-builder/issues/2679
    let shouldUnpack = false;
    // ffprobe-static and ffmpeg-static are known packages to always unpack
    const moduleName = path.basename(packageDir);
    if (moduleName === "ffprobe-static" || moduleName === "ffmpeg-static" || isLibOrExe(file)) shouldUnpack = true;
    else if (!file.includes(".", nextSlashIndex) && path.extname(file) === "") shouldUnpack = await isBinaryFile(file);
