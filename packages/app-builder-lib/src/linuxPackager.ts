...
export class LinuxPackager extends PlatformPackager<LinuxConfiguration> {
  ...
  constructor(info: Packager) {
    ...
    const { executableName } = this.platformSpecificBuildOptions;
    this.executableName =
  ...
  createTargets(targets: string[], mapper: (name: string, factory: (outDir: string) => Target) => void): void {
    ...
    const getHelper = () => {
      if (helper == null) helper = new LinuxTargetHelper(this);
    ...
    for (const name of targets) {
      ...
      mapper(name, (outDir) => {
        if (targetClass === null) return createCommonTarget(name, outDir, this);

        const target = new targetClass(name, this, getHelper(), outDir);
        if (process.platform === "win32" || process.env._REMOTE_BUILD) {
          if (remoteBuilder == null) remoteBuilder = new RemoteBuilder(this);
...
class RemoteTarget extends Target {
  ...
  private readonly buildTaskManager = new AsyncTaskManager(this.remoteBuilder.packager.info.cancellationToken);
