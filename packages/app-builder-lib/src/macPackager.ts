...
import { Arch, AsyncTaskManager, deepAssign, exec, InvalidConfigurationError, log, use } from "builder-util";
...
export default class MacPackager extends PlatformPackager<MacConfiguration> {
  ...
  readonly codeSigningInfo = new Lazy<CodeSigningInfo>(() => {
    ...
    return createKeychain({
      ...
      cscIKeyPassword: chooseNotNull(
        this.platformSpecificBuildOptions.cscInstallerKeyPassword,
        process.env.CSC_INSTALLER_KEY_PASSWORD,
      ),
      currentDir: this.projectDir,
    }).then((result) => {
      const { keychainFile } = result;
      if (keychainFile != null) this.info.disposeOnBuildFinish(() => removeKeychain(keychainFile));

      return result;
    });
  });

  private readonly _iconPath = new Lazy(() => this.getOrConvertIcon("icns"));
  ...
  async applyCommonInfo(appPlist: any, contentsPath: string) {
    ...
    appPlist.CFBundleExecutable = appFilename.endsWith(" Helper")
      ? appFilename.substring(0, appFilename.length - " Helper".length)
      : appFilename;
    ...
    if (icon != null) {
      ...
      if (oldIcon != null) await unlinkIfExists(path.join(resourcesPath, oldIcon));
    ...
    const { minimumSystemVersion } = this.platformSpecificBuildOptions;
    if (minimumSystemVersion != null) appPlist.LSMinimumSystemVersion = minimumSystemVersion;
    ...
    use(
      this.platformSpecificBuildOptions.category || (this.config as any).category,
      (it) => (appPlist.LSApplicationCategoryType = it),
    );
    appPlist.NSHumanReadableCopyright = appInfo.copyright;

    if (this.platformSpecificBuildOptions.darkModeSupport) appPlist.NSRequiresAquaSystemAppearance = false;

    const { extendInfo } = this.platformSpecificBuildOptions;
    if (extendInfo != null) Object.assign(appPlist, extendInfo);
  }

  createTargets(targets: string[], mapper: (name: string, factory: (outDir: string) => Target) => void): void {
    for (const name of targets)
      switch (name) {
        ...
        default:
          mapper(name, (outDir) =>
            name === "mas" || name === "mas-dev" ? new NoOpTarget(name) : createCommonTarget(name, outDir, this),
          );
          break;
      }
  }
  ...
  async pack(outDir: string, arch: Arch, targets: Target[], taskManager: AsyncTaskManager): Promise<any> {
    ...
    const { prepackaged } = this.packagerOptions;

    if (!hasMas || targets.length > 1) {
      const appPath =
        prepackaged == null
          ? path.join(this.computeAppOutDir(outDir, arch), `${this.appInfo.productFilename}.app`)
          : prepackaged;
    ...
    for (const target of targets) {
      ...
      if (!(targetName === "mas" || targetName === "mas-dev")) continue;
      ...
      } else await this.sign(prepackaged, targetOutDir, masBuildOptions);
    }

    if (nonMasPromise != null) await nonMasPromise;
  }

  // noinspection JSMethodCanBeStatic
  protected async doFlat(
    appPath: string,
    outFile: string,
    identity: Identity,
  ...
  protected async signApp(packContext: AfterPackContext, isAsar: boolean): Promise<any> {
    ...
    await BluebirdPromise.map(readdir(packContext.appOutDir), (file: string): any => {
      if (file === appFileName) return this.sign(path.join(packContext.appOutDir, file), null, null);
    ...
    await BluebirdPromise.map(orIfFileNotExist(readdir(outResourcesDir), []), (file: string): any => {
      if (file.endsWith(".app")) return this.sign(path.join(outResourcesDir, file), null, null);
  ...
  private async adjustSignOptions(signOptions: any, masOptions: MasConfiguration | null) {
    ...
    let { entitlements } = customSignOptions;
    if (entitlements == null) {
      const p = `entitlements.${entitlementsSuffix}.plist`;
      if (resourceList.includes(p)) entitlements = path.join(this.info.buildResourcesDir, p);
      else entitlements = getTemplatePath("entitlements.mac.plist");
    }
    signOptions.entitlements = entitlements;

    let { entitlementsInherit } = customSignOptions;
    if (entitlementsInherit == null) {
      const p = `entitlements.${entitlementsSuffix}.inherit.plist`;
      if (resourceList.includes(p)) entitlementsInherit = path.join(this.info.buildResourcesDir, p);
      else entitlementsInherit = getTemplatePath("entitlements.mac.plist");
    }
  ...
  private async sign(appPath: string, outDir: string | null, masOptions: MasConfiguration | null): Promise<void> {
    ...
    const { keychainFile } = await this.codeSigningInfo.value;
    ...
    if (!isMacOsHighSierra()) throw new InvalidConfigurationError("macOS High Sierra 10.13.6 is required to sign");
    ...
    const signOptions: any = {
      ...
      requirements:
        isMas || this.platformSpecificBuildOptions.requirements == null
          ? undefined
          : await this.getResource(this.platformSpecificBuildOptions.requirements),
    ...
    if (masOptions != null && !isDevelopment) {
      ...
      if (masInstallerIdentity == null)
        throw new InvalidConfigurationError(
          `Cannot find valid "${certType}" identity to sign MAS installer, please see https://deskgap.build/code-signing`,
        );
      ...
      await this.dispatchArtifactCreated(
        artifactPath,
        null,
        Arch.x64,
        this.computeSafeArtifactName(artifactName, "pkg"),
      );
    }
  }
}
...
function getCertificateType(isMas: boolean, isDevelopment: boolean): CertType {
  if (isDevelopment) return "Mac Developer";
