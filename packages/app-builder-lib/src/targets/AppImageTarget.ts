...
export default class AppImageTarget extends Target {
  ...
  constructor(
    ...
    this.desktopEntry = new Lazy<string>(() =>
      helper.computeDesktopEntry(this.options, "AppRun", {
  ...
  async build(appOutDir: string, arch: Arch): Promise<any> {
    const { packager } = this;
    const { options } = this;
    // https://github.com/deskgap-userland/deskgap-builder/issues/775
    // https://github.com/deskgap-userland/deskgap-builder/issues/1726
    ...
    if (publishConfig != null)
      await outputFile(
        path.join(packager.getResourcesDir(stageDir.dir), "app-update.yml"),
    ...
    if (packager.compression === "maximum") args.push("--compression", "xz");
