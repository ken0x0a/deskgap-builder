...
export default class FpmTarget extends Target {
  ...
  private readonly scriptFiles: Promise<string[]>;
  ...
  async build(appOutDir: string, arch: Arch): Promise<any> {
    ...
    const artifactPath = path.join(
      this.outDir,
      packager.expandArtifactNamePattern(this.options, target, arch, nameFormat, !isUseArchIfX64),
    );
    ...
    if (packager.packagerOptions.prepackaged != null) await ensureDir(this.outDir);
    ...
    const { packageCategory } = options;
    if (packageCategory != null) args.push("--category", packageCategory);

    if (target === "deb") use((options as DebOptions).priority, (it) => args.push("--deb-priority", it!));
    else if (target === "rpm")
      if (synopsis != null) {
    ...
    if (options.compression != null) fpmConfiguration.compression = options.compression;

    // noinspection JSDeprecatedSymbols
    const { depends } = options;
    if (depends != null)
      if (Array.isArray(depends)) {
        fpmConfiguration.customDepends = depends;
      } else {
        // noinspection SuspiciousTypeOfGuard
        if (typeof depends === "string") fpmConfiguration.customDepends = [depends as string];
        else throw new Error(`depends must be Array or String, but specified as: ${depends}`);
      }
    ...
    if (
      ...
      (await packager.packagerOptions.effectiveOptionComputed([args, desktopFilePath]))
    )
      return;
  ...
  private async computeFpmMetaInfoOptions(): Promise<FpmOptions> {
    ...
    if (projectUrl == null)
      errors.push(
        "Please specify project homepage, see https://deskgap.build/configuration/configuration#Metadata-homepage",
      );
    ...
    if (author == null) {
      ...
      if (a == null || a.email == null) errors.push(errorMessages.authorEmailIsMissed);
      else author = `${a.name} <${a.email}>`;
    }
  ...
  private async createScripts(): Promise<string[]> {
    ...
    return await Promise.all<string>([
      writeConfigFile(
        packager.info.tempDirManager,
        getResource(this.options.afterInstall, "after-install.tpl"),
        templateOptions,
      ),
      writeConfigFile(
        packager.info.tempDirManager,
        getResource(this.options.afterRemove, "after-remove.tpl"),
        templateOptions,
      ),
    ]);
  }
...
async function writeConfigFile(tmpDir: TmpDir, templatePath: string, options: any): Promise<string> {
  ...
  function replacer(match: string, p1: string) {
    if (p1 in options) return options[p1];
