...
import {
  ...
  getEffectiveOptions,
} from "../options/CommonWindowsInstallerConfiguration";
...
export default class MsiTarget extends Target {
  ...
  async build(appOutDir: string, arch: Arch) {
    ...
    const vendorPath = await getBinFromUrl(
      "wix",
      "4.0.0.5512.2",
      "/X5poahdCc3199Vt6AP7gluTlT1nxi9cbbHhZhCMEu+ngyP1LiBMn+oZX7QAZVaKeBMc2SjVp7fJqNLqsUnPNQ==",
    );

    // noinspection SpellCheckingInspection
    const candleArgs = [
      "-arch",
      arch === Arch.ia32 ? "x86" : arch === Arch.arm64 ? "arm64" : "x64",
      `-dappDir=${vm.toVmFile(appOutDir)}`,
    ].concat(this.getCommonWixArgs());
    candleArgs.push("project.wxs");
    if (uiFile !== null) candleArgs.push(ASSISTED_UI_FILE_NAME);
  ...
  private async computeFileDeclaration(appOutDir: string) {
    ...
    const files = await BluebirdPromise.map(walk(appOutDir), (file) => {
      ...
      if (lastSlash > 0) {
        ...
        // https://github.com/deskgap-userland/deskgap-builder/issues/3027
        directoryId = `d${createHash("md5")
          .update(dirName)
          .digest("base64")
          .replace(/\//g, "_")
          .replace(/\+/g, ".")
      ...
      } else if (!isRootDirAddedToRemoveTable) isRootDirAddedToRemoveTable = true;
      ...
      if (isMainExecutable) result += ' Id="mainExecutable"';
      else if (directoryId === null) result += ` Id="${path.basename(packagePath)}_f"`;
      ...
      if (isMainExecutable && (isCreateDesktopShortcut || commonOptions.isCreateStartMenuShortcut)) {
        result += `>\n`;
        const { shortcutName } = commonOptions;
        ...
        if (commonOptions.isCreateStartMenuShortcut) {
          if (hasMenuCategory)
            dirs.push(
              `<Directory Id="${startMenuShortcutDirectoryId}" Name="ProgramMenuFolder:\\${commonOptions.menuCategory}\\"/>`,
            );
  ...
  private getCommonWixArgs() {
    const args: string[] = ["-pedantic"];
    if (this.options.warningsAsErrors !== false) args.push("-wx");
  ...
  private async light(
    ...
    if (this.options.oneClick === false) lightArgs.push("-ext", "WixUIExtension");
  ...
  private async writeManifest(appOutDir: string, arch: Arch, commonOptions: FinalCommonWindowsInstallerOptions) {
    ...
    if (!companyName) log.warn(`Manufacturer is not set for MSI — please set "author" in the package.json`);

    const { compression } = this.packager;
    const { options } = this;
...
function listToString(list: string[], indentLevel: number) {
