...
export async function tar(
  ...
  if (format === "tar.lz") {
    ...
    let lzipPath = "lzip";
    if (process.platform === "darwin") lzipPath = path.join(await getLinuxToolsPath(), "bin", lzipPath);

    await exec(lzipPath, [
      compression === "store" ? "-1" : "-9",
...
export function compute7zCompressArgs(format: string, options: ArchiveOptions = {}) {
  ...
  if (!storeOnly) {
    ...
    if (!isLevelSet)
      // https://github.com/deskgap-userland/deskgap-builder/pull/3032
      args.push(`-mx=${!isZip || options.compression === "maximum" ? "9" : "7"}`);
  }

  if (options.dictSize != null) args.push(`-md=${options.dictSize}m`);
  ...
  if (!options.isRegularFile) args.push("-mtc=off");

  if (format === "7z" || format.endsWith(".7z")) {
    if (options.solid === false) args.push("-ms=off");

    if (options.isArchiveHeaderCompressed === false) args.push("-mhc=off");
  ...
  if (options.method != null) {
    if (options.method !== "DEFAULT") args.push(`-mm=${options.method}`);
  } else if (isZip || storeOnly) args.push(`-mm=${storeOnly ? "Copy" : "Deflate"}`);
...
export async function archive(
  ...
  if (options.excluded != null) for (const mask of options.excluded) args.push(`-xr!${mask}`);
...
function debug7zArgs(command: "a" | "x"): string[] {
  const args = [command, "-bd"];
  if (debug7z.enabled) args.push("-bb");
