...
export default class AppXTarget extends Target {
  private static async computeUserAssets(vm: VmManager, vendorPath: string, userAssetDir: string | null) {
    ...
    if (userAssetDir == null) userAssets = [];
    else {
      userAssets = (await readdir(userAssetDir)).filter(
        (it) => !it.startsWith(".") && !it.endsWith(".db") && it.includes("."),
      );
  ...
  async build(appOutDir: string, arch: Arch): Promise<any> {
    ...
    if (isScaledAssetsProvided(userAssets)) {
      ...
      await BluebirdPromise.map(assetInfo.allAssets, (it) =>
        copyOrLinkFile(it, path.join(assetRoot, path.basename(it))),
      );
    ...
    let mapping = "[Files]";
    for (const list of mappingList) mapping += `\r\n${list.join("\r\n")}`;

    await writeFile(mappingFile, mapping);
    packager.debugLogger.add("appx.mapping", mapping);

    if (this.options.makeappxArgs != null) makeAppXArgs.push(...this.options.makeappxArgs);
  ...
  private async computePublisherName() {
    ...
    if (publisher == null) throw new Error("Internal error: cannot compute subject using certificate info");

    return publisher;
  }

  private async getExtensions(executable: string, displayName: string): Promise<string> {
    const uriSchemes = asArray(this.packager.config.protocols).concat(
      asArray(this.packager.platformSpecificBuildOptions.protocols),
    );

    const fileAssociations = asArray(this.packager.config.fileAssociations).concat(
      asArray(this.packager.platformSpecificBuildOptions.fileAssociations),
    );
    ...
    if (isAddAutoLaunchExtension === undefined) {
      ...
      isAddAutoLaunchExtension = deps?.["deskgap-winstore-auto-launch"] != null;
    }

    if (
      !isAddAutoLaunchExtension &&
      uriSchemes.length === 0 &&
  ...
  private async writeManifest(outFile: string, arch: Arch, publisher: string, userAssets: string[]) {
    ...
    const manifest = (await readFile(path.join(getTemplatePath("appx"), "appxmanifest.xml"), "utf8")).replace(
      /\${([a-zA-Z0-9]+)}/g,
      (match, p1): string => {
        switch (p1) {
          case "publisher":
          ...
          case "publisherDisplayName": {
            ...
            if (name == null)
              throw new InvalidConfigurationError(
                `Please specify "author" in the application package.json — it is required because "appx.publisherDisplayName" is not set.`,
              );
...
function resourceLanguageTag(userLanguages: string[] | null | undefined): string {
  if (userLanguages == null || userLanguages.length === 0) userLanguages = [DEFAULT_RESOURCE_LANG];

  return userLanguages.map((it) => `<Resource Language="${it.replace(/_/g, "-")}" />`).join("\n");
}

function lockScreenTag(userAssets: string[]): string {
...
function defaultTileTag(userAssets: string[], showNameOnTiles: boolean): string {
  const defaultTiles: string[] = ["<uap:DefaultTile", 'Wide310x150Logo="assets\\Wide310x150Logo.png"'];

  if (isDefaultAssetIncluded(userAssets, "LargeTile.png"))
    defaultTiles.push('Square310x310Logo="assets\\LargeTile.png"');

  if (isDefaultAssetIncluded(userAssets, "SmallTile.png")) defaultTiles.push('Square71x71Logo="assets\\SmallTile.png"');
  ...
  } else defaultTiles.push("/>");

  return defaultTiles.join(" ");
}

function splashScreenTag(userAssets: string[]): string {
...
function isDefaultAssetIncluded(userAssets: string[], defaultAsset: string) {
...
function isScaledAssetsProvided(userAssets: string[]) {
