...
export class ArchiveTarget extends Target {
  ...
  async build(appOutDir: string, arch: Arch): Promise<any> {
    ...
    // tslint:disable-next-line:no-invalid-template-strings
    else defaultPattern = `\${productName}-\${version}${arch === Arch.x64 ? "" : "-${arch}"}-\${os}.\${ext}`;
    ...
    if (format.startsWith("tar."))
      ...
      if (isMac) {
        ...
        if (fileMatchers == null) dirToArchive = appOutDir;
        else {
      ...
      if (this.isWriteUpdateInfo && format === "zip") updateInfo = await appendBlockmap(artifactPath);
    }
