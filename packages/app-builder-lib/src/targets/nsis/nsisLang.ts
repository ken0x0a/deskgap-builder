...
export class LangConfigurator {
  ...
  constructor(options: NsisOptions) {
    ...
    else this.isMultiLang = options.multiLanguageInstaller !== false;

    if (this.isMultiLang)
      this.langs =
        rawList == null
          ? bundledLanguages.slice()
          : asArray(rawList).map((it) => toLangWithRegion(it.replace("-", "_")));
    else this.langs = ["en_US"];
  }
}
...
export function createAddLangsMacro(scriptGenerator: NsisScriptGenerator, langConfigurator: LangConfigurator) {
  ...
  for (const langWithRegion of langConfigurator.langs) {
    let name: string;
    if (langWithRegion === "zh_CN") name = "SimpChinese";
    else if (langWithRegion === "zh_TW") name = "TradChinese";
    else if (langWithRegion === "nb_NO") name = "Norwegian";
    else if (langWithRegion === "pt_BR") name = "PortugueseBR";
    else {
      const lang = langWithRegion.substring(0, langWithRegion.indexOf("_"));
      name = (langIdToName as any)[lang];
      if (name == null) throw new Error(`Language name is unknown for ${lang}`);

      if (name === "Spanish") name = "SpanishInternational";
    }
...
export async function addCustomMessageFileInclude(
  input: string,
...
function computeCustomMessageTranslations(messages: any, langConfigurator: LangConfigurator): string[] {
  const result: string[] = [];
  ...
  for (const messageId of Object.keys(messages)) {
    ...
    for (const lang of Object.keys(langToTranslations)) {
      ...
      if (!includedLangs.has(langWithRegion)) continue;

      const value = langToTranslations[lang];
      if (value == null) throw new Error(`${messageId} not specified for ${lang}`);
