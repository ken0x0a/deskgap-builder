...
} from "../differentialUpdateInfoBuilder";
...
// noinspection SpellCheckingInspection
const nsisResourcePathPromise = new Lazy(() =>
  getBinFromUrl(
    "nsis-resources",
    "3.4.1",
...
export class NsisTarget extends Target {
  ...
  constructor(
    ...
    if (deps?.["deskgap-squirrel-startup"] != null)
  ...
  async buildAppPackage(appOutDir: string, arch: Arch): Promise<PackageFileInfo> {
    ...
    const { isBuildDifferentialAware } = this;
    const format = !isBuildDifferentialAware && options.useZip ? "zip" : "7z";
    const archiveFile = path.join(
      this.outDir,
      `${packager.appInfo.sanitizedName}-${packager.appInfo.version}-${Arch[arch]}.nsis.${format}`,
    );
    ...
    await archive(
      ...
      archiveFile,
      appOutDir,
      isBuildDifferentialAware ? configureDifferentialAwareArchiveOptions(archiveOptions) : archiveOptions,
    );
    timer.end();
  ...
  protected configureDefines(oneClick: boolean, defines: any): Promise<any> {
    ...
    if (oneClick) {
      ...
      if (options.runAfterFinish !== false) defines.RUN_AFTER_FINISH = null;
      ...
      asyncTaskManager.add(async () => {
        ...
        if (installerHeaderIcon != null) defines.HEADER_ICO = installerHeaderIcon;
      });
    } else {
      if (options.runAfterFinish === false) defines.HIDE_RUN_AFTER_FINISH = null;
      ...
      asyncTaskManager.add(async () => {
        const bitmap =
          (await packager.getResource(options.installerSidebar, "installerSidebar.bmp")) ||
          "${NSISDIR}\\Contrib\\Graphics\\Wizard\\nsis3-metro.bmp";
        defines.MUI_WELCOMEFINISHPAGE_BITMAP = bitmap;
        defines.MUI_UNWELCOMEFINISHPAGE_BITMAP =
    ...
    if (!oneClick || options.perMachine === true) defines.INSTALL_MODE_PER_ALL_USERS_REQUIRED = null;
    ...
    if (commonOptions.menuCategory != null) defines.MENU_FILENAME = commonOptions.menuCategory;

    defines.SHORTCUT_NAME = commonOptions.shortcutName;

    if (options.deleteAppDataOnUninstall) defines.DELETE_APP_DATA_ON_UNINSTALL = null;
    ...
    defines.UNINSTALL_DISPLAY_NAME = packager.expandMacro(
    ...
    if (!commonOptions.isCreateStartMenuShortcut) defines.DO_NOT_CREATE_START_MENU_SHORTCUT = null;

    if (options.displayLanguageSelector === true) defines.DISPLAY_LANG_SELECTOR = null;
  ...
  private async buildInstaller(): Promise<any> {
    ...
    const { isPortable } = this;
    const iconPath =
      (isPortable ? null : await packager.getResource(options.installerIcon, "installerIcon.ico")) ||
    ...
    if (this.isPortable && options.useZip)
      ...
      await BluebirdPromise.map(this.archs.keys(), async (arch) => {
        ...
        if (match == null) log.warn({ output: archiveInfo }, "cannot compute size of app package");
        else estimatedSize += parseInt(match[1], 10);
      });
    ...
    if (isPortable) {
      ...
      if (portableOptions.splashImage != null) defines.SPLASH_IMAGE = portableOptions.splashImage;
    } else await this.configureDefines(oneClick, defines);

    if (estimatedSize !== 0)
      // in kb
      defines.ESTIMATED_SIZE = Math.round(estimatedSize / 1024);

    if (packager.compression === "store") commands.SetCompress = "off";
    else {
      ...
      if (!this.isWebInstaller) defines.COMPRESS = "auto";
    }
    ...
    if (
      ...
      (await packager.packagerOptions.effectiveOptionComputed([defines, commands]))
    )
      return;
    ...
    const script = isPortable
      ...
      : await this.computeScriptAndSignUninstaller(
          definesUninstaller,
          commandsUninstaller,
          installerPath,
          sharedHeader,
    ...
    await Promise.all<any>([
      packager.sign(installerPath),
      defines.UNINSTALLER_OUT_FILE == null ? Promise.resolve() : unlink(defines.UNINSTALLER_OUT_FILE),
    ]);

    const safeArtifactName = computeSafeArtifactNameIfNeeded(installerFilename, () =>
      this.generateGitHubInstallerName(),
    );
    let updateInfo: any;
    if (this.isWebInstaller) updateInfo = createNsisWebDifferentialUpdateInfo(installerPath, packageFiles);
    else if (this.isBuildDifferentialAware)
      updateInfo = await createBlockmap(installerPath, this, packager, safeArtifactName);

    if (updateInfo != null && isPerMachine && oneClick) updateInfo.isAdminRightsRequired = true;
  ...
  private async computeCommonInstallerScriptHeader() {
    ...
    taskManager.add(async () => {
      ...
      if (stat?.isDirectory()) scriptGenerator.addPluginDir(pluginArch, userPluginDir);
    });
    ...
    if (!this.isPortable) {
      if (options.oneClick === false)
        taskManager.addTask(
          addCustomMessageFileInclude("assistedMessages.yml", packager, scriptGenerator, langConfigurator),
        );
  ...
  private async computeFinalScript(originalScript: string, isInstaller: boolean) {
    ...
    if (this.isPortable) return scriptGenerator.build() + originalScript;
      ...
      if (isInstaller) {
        ...
        for (const item of fileAssociations) {
          ...
          for (const ext of extensions) {
            const customIcon = await packager.getResource(
              getPlatformIconFileName(item.icon, false),
              `${extensions[0]}.ico`,
            );
            ...
            registerFileAssociationsScript.insertMacro(
              "APP_ASSOCIATE",
              `"${ext}" "${item.name || ext}" "${item.description || ""}" ${icon} ${commandText} ${command}`,
            );
          }
        }
        ...
        for (const item of fileAssociations)
          for (const ext of asArray(item.ext))
            unregisterFileAssociationsScript.insertMacro(
              "APP_UNASSOCIATE",
              `"${normalizeExt(ext)}" "${item.name || ext}"`,
            );
  ...
  private async computeScriptAndSignUninstaller(
    ...
    else await execWine(installerPath);
  ...
  private computeVersionKey(short: boolean = false) {
    ...
    use(this.packager.platformSpecificBuildOptions.legalTrademarks, (it) =>
      versionKey.push(`/LANG=${localeId} LegalTrademarks "${it}"`),
    );
  ...
  private configureDefinesForAllTypeOfInstaller(defines: any) {
    const { appInfo } = this.packager;
    const { companyName } = appInfo;
    if (companyName != null) defines.COMPANY_NAME = companyName;

    // deskgap uses product file name as app data, define it as well to remove on uninstall
    if (defines.APP_FILENAME !== appInfo.productFilename) defines.APP_PRODUCT_FILENAME = appInfo.productFilename;

    if (this.isWebInstaller)
      defines.APP_PACKAGE_STORE_FILE = `${appInfo.updaterCacheDirName}\\${CURRENT_APP_PACKAGE_FILE_NAME}`;
    else defines.APP_INSTALLER_STORE_FILE = `${appInfo.updaterCacheDirName}\\${CURRENT_APP_INSTALLER_FILE_NAME}`;

    if (!this.isWebInstaller && defines.APP_BUILD_DIR == null) {
      const { options } = this;
      if (options.useZip) defines.ZIP_COMPRESSION = null;
  ...
  private async executeMakensis(defines: any, commands: any, script: string) {
    const args: string[] = this.options.warningsAsErrors === false ? [] : ["-WX"];
    for (const name of Object.keys(defines)) {
      const value = defines[name];
      if (value == null) args.push(`-D${name}`);
      else args.push(`-D${name}=${value}`);
    }

    for (const name of Object.keys(commands)) {
      const value = commands[name];
      if (Array.isArray(value)) for (const c of value) args.push(`-X${name} ${c}`);
      else args.push(`-X${name} ${value}`);
    }

    args.push("-");

    if (this.packager.debugLogger.isEnabled) this.packager.debugLogger.add("nsis.script", script);
  ...
  private getPreCompressedFileExtensions(): string[] | null {
...
async function generateForPreCompressed(
