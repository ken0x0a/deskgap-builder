...
export const NSIS_PATH = new Lazy(() => {
  ...
  if (custom != null && custom.length > 0) return Promise.resolve(custom.trim());
...
export class AppPackageHelper {
  ...
  async finishBuild(): Promise<any> {
    ...
    for (const [info, isDelete] of this.infoToIsDelete.entries()) if (isDelete) filesToDelete.push(info.path);
  ...
  async packArch(arch: Arch, target: NsisTarget): Promise<PackageFileInfo> {
    ...
    if (target.isWebInstaller) this.infoToIsDelete.set(info, false);
    else if (!this.infoToIsDelete.has(info)) this.infoToIsDelete.set(info, true);
...
export class CopyElevateHelper {
  ...
  copy(appOutDir: string, target: NsisTarget): Promise<any> {
    if (!target.packager.info.framework.isCopyElevateHelper) return Promise.resolve();
    ...
    if (isPackElevateHelper === false) return Promise.resolve();
...
export class UninstallerReader {
  ...
  static exec(installerPath: string, uninstallerPath: string) {
    ...
    // IMAGE_DOS_HEADER
    if (!reader.match([0x4d, 0x5a])) throw new Error("Invalid 'MZ' signature.");

    reader.skip(58);
    // e_lfanew
    reader.skip(reader.uint32() - reader.position);
    // IMAGE_FILE_HEADER
    if (!reader.match([0x50, 0x45, 0x00, 0x00])) throw new Error("Invalid 'PE' signature.");
    ...
    for (let i = 0; i < numberOfSections; i++) {
      ...
      switch (name) {
        ...
        default: {
          if (rawPointer !== 0 && rawSize !== 0) throw new Error(`Unsupported section '${name}'.`);
    ...
    nsisReader.uint32(); // ?
    if (!nsisReader.match(nsisSignature)) throw new Error("Invalid signature.");

    nsisReader.uint32(); // ?
    if (nsisSize !== nsisReader.uint32()) throw new Error("Size mismatch.");
    ...
    while (true) {
      ...
      if (compressed) buffer = zlib.inflateRawSync(buffer);

      const innerReader = new BinaryReader(buffer);
      innerReader.uint32(); // ?
      if (innerReader.match(nsisSignature)) {
        if (innerBuffer) throw new Error("Multiple inner blocks.");

        innerBuffer = buffer;
      }
    }
    if (!innerBuffer) throw new Error("Inner block not found.");
