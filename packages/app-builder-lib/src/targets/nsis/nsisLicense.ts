...
export async function computeLicensePage(
  ...
  if (license != null) {
    ...
    else licensePage = [`!insertmacro MUI_PAGE_LICENSE "${license}"`];
  ...
  for (const item of licenseFiles) {
    ...
    if (defaultFile == null) defaultFile = item.file;

    licensePage.push(`LicenseLangString MUILicense ${lcid[item.langWithRegion] || item.lang} "${item.file}"`);
  }

  for (const l of unspecifiedLangs) licensePage.push(`LicenseLangString MUILicense ${lcid[l]} "${defaultFile}"`);
