...
export class LinuxTargetHelper {
  ...
  constructor(private readonly packager: LinuxPackager) {}

  async computeDesktopEntry(
    targetSpecificOptions: LinuxTargetSpecificOptions,
    exec?: string,
    extra?: { [key: string]: string },
  ): Promise<string> {
    if (exec != null && exec.length === 0) throw new Error("Specified exec is empty");

    // https://github.com/deskgap-userland/deskgap-builder/issues/3418
    if (targetSpecificOptions.desktop?.Exec != null)
    ...
    const { executableArgs } = targetSpecificOptions;
    if (exec == null) {
      exec = `${installPrefix}/${productFilename}/${packager.executableName}`;
      if (!/^[/0-9A-Za-z._-]+$/.test(exec)) exec = `"${exec}"`;
    ...
    if (!isEmptyOrSpaces(description)) desktopMeta.Comment = description;

    const mimeTypes: string[] = asArray(targetSpecificOptions.mimeTypes);
    for (const fileAssociation of packager.fileAssociations)
      if (fileAssociation.mimeType != null) mimeTypes.push(fileAssociation.mimeType);

    for (const protocol of asArray(packager.config.protocols).concat(
      asArray(packager.platformSpecificBuildOptions.protocols),
    ))
      for (const scheme of asArray(protocol.schemes)) mimeTypes.push(`x-scheme-handler/${scheme}`);

    if (mimeTypes.length !== 0) desktopMeta.MimeType = `${mimeTypes.join(";")};`;

    let { category } = targetSpecificOptions;
    if (isEmptyOrSpaces(category)) {
      const macCategory = (packager.config.mac || {}).category;
      if (macCategory != null) category = macToLinuxCategory[macCategory];
    ...
    for (const name of Object.keys(desktopMeta)) data += `\n${name}=${desktopMeta[name]}`;
  ...
  private async computeDesktopIcons(): Promise<IconInfo[]> {
    const { packager } = this;
    ...
    if (icnsPath != null) sources.push(icnsPath);
  ...
  private async computeMimeTypeFiles(): Promise<string | null> {
    ...
    for (const fileAssociation of this.packager.fileAssociations) {
      if (!fileAssociation.mimeType) continue;
