...
// https://github.com/deskgap-userland/deskgap-osx-sign/issues/96#issuecomment-274986942
export class PkgTarget extends Target {
  ...
  async build(appPath: string, arch: Arch): Promise<any> {
    ...
    if (identity == null && packager.forceCodeSigning) {
      throw new Error(`Cannot find valid "${certType}" to sign standalone installer, please see https://deskgap.build/code-signing`)
    }
  ...
  private async buildComponentPackage(appPath: string, propertyListOutputFile: string, packageOutputFile: string) {
    ...
    const plistInfo = (await executeAppBuilderAsJson<Array<any>>(["decode-plist", "-f", propertyListOutputFile]))[0].filter((it: any) => it.RootRelativeBundlePath !== "DeskGap.dSYM")
    if (plistInfo.length > 0) {
      const packageInfo = plistInfo[0]

      // ChildBundles lists all of deskgap binaries within the .app.
      // There is no particular reason for removing that key, except to be as close as possible to
      // the PackageInfo generated by previous versions of deskgap-builder.
