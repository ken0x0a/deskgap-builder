...
export class PkgTarget extends Target {
  ...
  async build(appPath: string, arch: Arch): Promise<any> {
    ...
    if (identity == null && packager.forceCodeSigning)
      throw new Error(
        `Cannot find valid "${certType}" to sign standalone installer, please see https://deskgap.build/code-signing`,
      );
    ...
    await packager.dispatchArtifactCreated(
      artifactPath,
      this,
      arch,
      packager.computeSafeArtifactName(artifactName, "pkg", arch),
    );
  }
  ...
  private async buildComponentPackage(appPath: string, propertyListOutputFile: string, packageOutputFile: string) {
    ...
    const plistInfo = (await executeAppBuilderAsJson<any[]>(["decode-plist", "-f", propertyListOutputFile]))[0].filter(
      (it: any) => it.RootRelativeBundlePath !== "DeskGap.dSYM",
    );
    if (plistInfo.length > 0) {
      ...
      if (options.isRelocatable != null) packageInfo.BundleIsRelocatable = options.isRelocatable;

      if (options.isVersionChecked != null) packageInfo.BundleIsVersionChecked = options.isVersionChecked;

      if (options.hasStrictIdentifier != null) packageInfo.BundleHasStrictIdentifier = options.hasStrictIdentifier;

      if (options.overwriteAction != null) packageInfo.BundleOverwriteAction = options.overwriteAction;
    ...
    if (options.scripts != null)
      ...
      if (stat?.isDirectory()) args.push("--scripts", dir);
    }
  ...
  private async customizeDistributionConfiguration(distInfoFile: string, appPath: string) {
    ...
    distInfo = `${distInfo.substring(0, insertIndex)}    <domains enable_anywhere="${
      options.allowAnywhere
    }" enable_currentUserHome="${options.allowCurrentUserHome}" enable_localSystem="${
      options.allowRootDirectory
    ...
    if (options.background != null) {
      ...
      if (background != null) {
        ...
        distInfo = `${distInfo.substring(
          0,
          insertIndex,
        )}    <background file="${background}" alignment="${alignment}" scaling="${scaling}"/>\n${distInfo.substring(
          insertIndex,
        )}`;
        distInfo = `${distInfo.substring(
          0,
          insertIndex,
        )}    <background-darkAqua file="${background}" alignment="${alignment}" scaling="${scaling}"/>\n${distInfo.substring(
          insertIndex,
        )}`;
      }
    }
...
export function prepareProductBuildArgs(identity: Identity | null, keychain: string | null | undefined): string[] {
  const args: string[] = [];
  if (identity != null) {
