...
import { Arch, asArray, executeAppBuilder, InvalidConfigurationError, log, use } from "builder-util";
...
export class WinPackager extends PlatformPackager<WindowsConfiguration> {
  ...
  readonly computedPublisherName = new Lazy<string[] | null>(async () => {
    const { publisherName } = this.platformSpecificBuildOptions;
    if (publisherName === null) return null;

    if (publisherName != null) return asArray(publisherName);
  ...
  readonly cscInfo = new Lazy<FileCodeSigningInfo | CertificateFromStoreInfo | null>(() => {
    const { platformSpecificBuildOptions } = this;
    if (
      platformSpecificBuildOptions.certificateSubjectName != null ||
      platformSpecificBuildOptions.certificateSha1 != null
    )
      return this.vm.value
        ...
        .catch((e) => {
          ...
          if (platformSpecificBuildOptions.sign == null) throw e;
          else {
    ...
    const { certificateFile } = platformSpecificBuildOptions;
    ...
    if (cscLink == null) return Promise.resolve(null);
  ...
  readonly lazyCertInfo = new Lazy<CertificateInfo | null>(async () => {
    ...
    if (cscFile == null) return null;

    return getCertInfo(cscFile, cscInfo.password || "");
  });

  readonly vm = new Lazy<VmManager>(() =>
    process.platform === "win32" ? Promise.resolve(new VmManager()) : getWindowsVm(this.debugLogger),
  );

  private readonly _iconPath = new Lazy(() => this.getOrConvertIcon("ico"));
  ...
  createTargets(targets: string[], mapper: (name: string, factory: (outDir: string) => Target) => void): void {
    ...
    const getCopyElevateHelper = () => {
      if (copyElevateHelper == null) copyElevateHelper = new CopyElevateHelper();
    ...
    const getHelper = () => {
      if (helper == null) helper = new AppPackageHelper(getCopyElevateHelper());

      return helper;
    };

    for (const name of targets) {
      if (name === DIR_TARGET) continue;
      ...
      if (name === "nsis" || name === "portable")
        ...
        mapper(
          name,
          (outDir) =>
            new WebInstallerTarget(this, path.join(outDir, name), name, new AppPackageHelper(getCopyElevateHelper())),
        );
      else {
            ...
            case "squirrel":
              try {
                ...
                throw new InvalidConfigurationError(
                  `Module deskgap-builder-squirrel-windows must be installed in addition to build Squirrel.Windows: ${
                    e.stack || e
                  }`,
                );
              }
        ...
        mapper(name, (outDir) =>
          targetClass === null ? createCommonTarget(name, outDir, this) : new (targetClass as any)(this, outDir, name),
        );
      }
    }
  }
  ...
  async sign(file: string, logMessagePrefix?: string): Promise<void> {
    ...
    const cscInfo = await this.cscInfo.value;
    if (cscInfo == null) {
      if (this.platformSpecificBuildOptions.sign != null) await sign(signOptions, this);
      else if (this.forceCodeSigning)
        throw new InvalidConfigurationError(
          `App is not signed and "forceCodeSigning" is set to true, please ensure that code signing configuration is correct, please see https://deskgap.build/code-signing`,
        );

      return;
    }

    if (logMessagePrefix == null) logMessagePrefix = "signing";
  ...
  async signAndEditResources(
    ...
    use(this.platformSpecificBuildOptions.legalTrademarks, (it) =>
      args.push("--set-version-string", "LegalTrademarks", it!),
    );
    ...
    const { config } = this;
    const cscInfoForCacheDigest =
      !isBuildCacheEnabled() || isCI || config.deskgapDist != null ? null : await this.cscInfo.value;
    ...
    if (cscInfoForCacheDigest != null) {
      ...
      if (cscFile != null) files.push(cscFile);
    ...
    if (buildCacheManager != null) await buildCacheManager.save();
  }

  protected createTransformerForExtraFiles(packContext: AfterPackContext): FileTransformer | null {
    if (this.platformSpecificBuildOptions.signAndEditExecutable === false) return null;

    return (file) => {
      if (file.endsWith(".exe") || (this.isSignDlls() && file.endsWith(".dll"))) {
        const parentDir = path.dirname(file);
        if (parentDir !== packContext.appOutDir) return new CopyFileTransformer((file) => this.sign(file));
      }
      return null;
    };
  }
  ...
  protected doGetCscPassword(): string | undefined | null {
    return chooseNotNull(
      chooseNotNull(this.platformSpecificBuildOptions.certificatePassword, process.env.WIN_CSC_KEY_PASSWORD),
  ...
  protected async signApp(packContext: AfterPackContext, isAsar: boolean): Promise<any> {
    ...
    if (this.platformSpecificBuildOptions.signAndEditExecutable === false) return;
    ...
    const fileToSign = await walk(
      outResourcesDir,
      (file, stat) => stat.isDirectory() || file.endsWith(".exe") || file.endsWith(".dll"),
    );
  ...
  private async doSign(options: WindowsSignOptions) {
    for (let i = 0; i < 3; i++)
      try {
        ...
        const { message } = e;
        if (message?.includes("Couldn't resolve host name")) {
