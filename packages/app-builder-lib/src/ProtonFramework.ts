...
export class ProtonFramework extends LibUiFramework {
  ...
  createTransformer(): FileTransformer | null {
    ...
    return (file): Promise<any> | null => {
      if (!(file.endsWith(".js") || file.endsWith(".jsx")) || file.includes(NODE_MODULES_PATTERN)) return null;

      return new Promise((resolve, reject) => {
        return babel.transformFile(file, babelOptions, (error: Error, result: any) => {
          if (error == null) resolve(result.code);
          else reject(error);
        });
      });
    };
  ...
  getDefaultIcon(platform: Platform): string {
    if (platform === Platform.WINDOWS) return getTemplatePath("icons/proton-native/proton-native.ico");

    if (platform === Platform.LINUX) return getTemplatePath("icons/proton-native/linux");
