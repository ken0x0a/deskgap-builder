...
export class AppInfo {
  get channel(): string | null {
    ...
    if (prereleaseInfo != null && prereleaseInfo.length > 0) return prereleaseInfo[0];
  ...
  get copyright(): string {
    const { copyright } = this.info.config;
    if (copyright != null) return expandMacro(copyright, null, this);
  ...
  get id(): string {
    ...
    for (const options of [this.platformSpecificOptions, this.info.config])
      if (options != null && appId == null) appId = options.appId;
  ...
  get linuxPackageName(): string {
    const { name } = this;
    // https://github.com/deskgap-userland/deskgap-builder/issues/2963
  ...
  constructor(
    ...
    if (info.metadata.shortVersion) this.shortVersion = info.metadata.shortVersion;

    if (info.metadata.shortVersionWindows) this.shortVersionWindows = info.metadata.shortVersionWindows;
  ...
  getVersionInWeirdWindowsForm(isSetBuildNumber = true): string {
    ...
    // https://github.com/deskgap-userland/deskgap-builder/issues/2635#issuecomment-371792272
    let buildNumber = isSetBuildNumber ? this.buildNumber : null;
    if (buildNumber == null || !/^\d+$/.test(buildNumber)) buildNumber = "0";
