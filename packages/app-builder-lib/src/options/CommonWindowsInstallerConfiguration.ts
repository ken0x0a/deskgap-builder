...
export function getEffectiveOptions(
  ...
  if (options.menuCategory != null && options.menuCategory !== false)
    if (options.menuCategory === true) {
      const { companyName } = packager.appInfo;
      if (companyName == null)
        throw new InvalidConfigurationError(
          `Please specify "author" in the application package.json — it is required because "menuCategory" is set to true.`,
        );
  ...
  return {
    ...
    shortcutName: isEmptyOrSpaces(options.shortcutName)
      ? appInfo.productFilename
      : packager.expandMacro(options.shortcutName),
...
function convertToDesktopShortcutCreationPolicy(value: boolean | undefined | string): DesktopShortcutCreationPolicy {
  if (value === false) return DesktopShortcutCreationPolicy.NEVER;

  if (value === "always") return DesktopShortcutCreationPolicy.ALWAYS;
