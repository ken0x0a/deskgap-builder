...
import { Arch, Packager, PackagerOptions, PublishOptions, UploadTask } from "..";
import SnapTarget from "../targets/snap";

if (process.env.BUILDER_REMOVE_STAGE_EVEN_IF_DEBUG == null) process.env.BUILDER_REMOVE_STAGE_EVEN_IF_DEBUG = "true";
...
async function doBuild(data: BuildTask): Promise<void> {
  ...
  if (projectDir == null) throw new InvalidConfigurationError("Env PROJECT_DIR must be set for builder process");

  const { targets } = data;
  if (data.platform == null) throw new InvalidConfigurationError("platform not specified");

  if (targets == null) throw new InvalidConfigurationError("targets path not specified");

  if (!Array.isArray(targets)) throw new InvalidConfigurationError("targets must be array of target name");
  ...
  if (projectDir == null) throw new InvalidConfigurationError("Env PROJECT_OUT_DIR must be set for builder process");
...
doBuild(JSON.parse(process.argv[2])).catch((error) => {
  process.exitCode = 0;
  return writeFile(
    path.join(process.env.APP_BUILDER_TMP_DIR!, "__build-result.json"),
