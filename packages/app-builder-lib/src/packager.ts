...
import { computeDefaultAppDirectory, getConfig, PACKAGE_VERSION, validateConfig } from "./util/config";
...
async function createFrameworkInfo(configuration: Configuration, packager: Packager): Promise<Framework> {
  let { framework } = configuration;
  if (framework != null) framework = framework.toLowerCase();

  let { nodeVersion } = configuration;
  if (framework === "deskgap" || framework == null) return await createDeskGapFrameworkSupport(configuration, packager);

  if (nodeVersion == null || nodeVersion === "current") nodeVersion = process.versions.node;
  ...
  if (framework === "libui") return new LibUiFramework(nodeVersion, distMacOsName, isUseLaunchUi);
...
export class Packager {
  ...
  private readonly afterPackHandlers: ((context: AfterPackContext) => Promise<any> | null)[] = [];

  private readonly nodeDependencyInfo = new Map<string, Lazy<any[]>>();

  private readonly toDispose: (() => Promise<void>)[] = [];
  ...
  constructor(options: PackagerOptions, readonly cancellationToken = new CancellationToken()) {
    ...
    const targets = options.targets || new Map<Platform, Map<Arch, string[]>>();
    if (options.targets == null) options.targets = targets;

    function processTargets(platform: Platform, types: string[]) {
      function commonArch(currentIfNotSpecified: boolean): Arch[] {
        if (platform === Platform.MAC) return currentIfNotSpecified ? [Arch.x64] : [];
      ...
      if (archToType == null) {
        archToType = new Map<Arch, string[]>();
        targets.set(platform, archToType);
      }

      if (types.length === 0) {
        for (const arch of commonArch(false)) archToType.set(arch, []);
      ...
      for (const type of types) {
        ...
        else for (const arch of commonArch(true)) addValue(archToType, arch, type);
      }
    }

    if (options.mac != null) processTargets(Platform.MAC, options.mac);

    if (options.linux != null) processTargets(Platform.LINUX, options.linux);

    if (options.win != null) processTargets(Platform.WINDOWS, options.win);
  ...
  async _build(
    ...
    if (repositoryInfo != null) this._repositoryInfo.value = Promise.resolve(repositoryInfo);
    ...
    this.artifactCreated((event) => {
      if (event.file != null) artifactPaths.add(event.file);
    });
  ...
  async afterPack(context: AfterPackContext): Promise<any> {
    ...
    for (const handler of handlers) await Promise.resolve(handler(context));
  }
  ...
  async build(): Promise<BuildResult> {
    ...
    } else if (configFromOptions?.extends?.includes(".")) {
    ...
    if (log.isDebugEnabled) log.debug({ config: getSafeEffectiveConfig(configuration) }, "effective config");
    ...
    if (this.devMetadata != null && !this.isTwoPackageJsonProjectLayoutUsed) this._metadata = this.devMetadata;
    else this._metadata = await this.readProjectMetadataIfTwoPackageStructureOrPrepacked(appPackageFile);
  ...
  async callAppxManifestCreated(path: string): Promise<void> {
    ...
    if (handler != null) await Promise.resolve(handler(path));
  }
  ...
  async callArtifactBuildCompleted(event: ArtifactCreated): Promise<void> {
    ...
    if (handler != null) await Promise.resolve(handler(event));
  }
  ...
  getNodeDependencyInfo(platform: Platform | null): Lazy<NodeModuleDirInfo[]> {
    let key = "";
    ...
    if (platform != null && this.framework.getExcludedDependencies != null) {
      ...
      if (excludedDependencies != null) key += `-${platform.name}`;
    }
  ...
  private createHelper(platform: Platform): PlatformPackager<any> {
    if (this.options.platformPackagerFactory != null) return this.options.platformPackagerFactory(this, platform);
  ...
  private async doBuild(): Promise<Map<Platform, Map<string, Target>>> {
    ...
    for (const [platform, archToType] of this.options.targets!) {
      ...
      for (const target of nameToTarget.values()) taskManager.addTask(target.finishBuild());
    }
  ...
  private async installAppDependencies(platform: Platform, arch: Arch): Promise<any> {
    if (this.options.prepackaged != null || this.framework.isNpmRebuildRequired !== true) return;
...
function createOutDirIfNeed(targetList: Target[], createdOutDirs: Set<string>): Promise<any> {
  const ourDirs = new Set<string>();
  for (const target of targetList) {
    // noinspection SuspiciousInstanceOfGuard
    if (target instanceof NoOpTarget) continue;

    const { outDir } = target;
    if (!createdOutDirs.has(outDir)) ourDirs.add(outDir);
  }

  if (ourDirs.size === 0) return Promise.resolve();
...
function getSafeEffectiveConfig(configuration: Configuration): string {
  ...
  if (o.cscLink != null) o.cscLink = "<hidden by builder>";
