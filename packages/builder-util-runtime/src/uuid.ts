...
export class UUID {
  ...
  static check(uuid: Buffer | string, offset: number = 0) {
    ...
    if (Buffer.isBuffer(uuid)) {
      ...
      if (i === 16) return { version: undefined, variant: "nil", format: "binary" };
  ...
  constructor(uuid: Buffer | string) {
    ...
    if (check.format === "ascii") this.ascii = uuid as string;
    else this.binary = uuid as Buffer;
  }
...
function uuidNamed(
  ...
  version: number,
  namespace: Buffer,
  encoding: UuidEncoding = UuidEncoding.ASCII,
) {
  const hash = createHash(hashMethod);

  const nameIsNotAString = typeof name !== "string";
  if (nameIsNotAString && !Buffer.isBuffer(name)) throw newError(invalidName, "ERR_INVALID_UUID_NAME");
  ...
  switch (encoding) {
    ...
    case UuidEncoding.OBJECT:
      ...
      break;
    default:
      result = `${byte2hex[buffer[0]] + byte2hex[buffer[1]] + byte2hex[buffer[2]] + byte2hex[buffer[3]]}-${
        byte2hex[buffer[4]]
      }${byte2hex[buffer[5]]}-${byte2hex[(buffer[6] & 0x0f) | version]}${byte2hex[buffer[7]]}-${
        byte2hex[(buffer[8] & 0x3f) | 0x80]
      }${byte2hex[buffer[9]]}-${byte2hex[buffer[10]]}${byte2hex[buffer[11]]}${byte2hex[buffer[12]]}${
        byte2hex[buffer[13]]
...
function stringify(buffer: Buffer) {
  return `${byte2hex[buffer[0]] + byte2hex[buffer[1]] + byte2hex[buffer[2]] + byte2hex[buffer[3]]}-${
    byte2hex[buffer[4]]
  }${byte2hex[buffer[5]]}-${byte2hex[buffer[6]]}${byte2hex[buffer[7]]}-${byte2hex[buffer[8]]}${byte2hex[buffer[9]]}-${
    byte2hex[buffer[10]]
  }${byte2hex[buffer[11]]}${byte2hex[buffer[12]]}${byte2hex[buffer[13]]}${byte2hex[buffer[14]]}${byte2hex[buffer[15]]}`;
}
