...
export class BintrayClient {
  ...
  constructor(
    ...
    if (options.owner == null) throw new Error("owner is not specified");

    if (options.package == null) throw new Error("package is not specified");
  ...
  deleteVersion(version: string): Promise<any> {
    return this.bintrayRequest(
      `${this.basePath}/versions/${version}`,
      this.auth,
      null,
      this.cancellationToken,
      "DELETE",
  ...
  getVersionFiles(version: string): Promise<File[]> {
    return this.bintrayRequest<File[]>(
      `${this.basePath}/versions/${version}/files`,
      this.auth,
      null,
      this.cancellationToken,
    );
    ...
    return parseJson(
      this.httpExecutor.request(
        configureRequestOptions(
          { hostname: "api.bintray.com", path, headers: this.requestHeaders || undefined },
          auth,
          method,
        ),
        cancellationToken,
