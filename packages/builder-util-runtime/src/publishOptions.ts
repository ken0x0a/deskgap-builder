...
export interface BaseS3Options extends PublishConfiguration {
  /**
   * The ACL. Set to `null` to not [add](https://github.com/deskgap-userland/deskgap-builder/issues/1822).
   *
   * @default public-read
   */
...
export interface S3Options extends BaseS3Options {
   ...
   * The ACL. Set to `null` to not [add](https://github.com/deskgap-userland/deskgap-builder/issues/1822).
   *
   * Please see [required permissions for the S3 provider](https://github.com/deskgap-userland/deskgap-builder/issues/1618#issuecomment-314679128).
   *
   * @default public-read
   */
  readonly acl?: "private" | "public-read" | null;
...
export function getS3LikeProviderBaseUrl(configuration: PublishConfiguration) {
  ...
  if (provider === "s3") return s3Url(configuration as S3Options);

  if (provider === "spaces") return spacesUrl(configuration as SpacesOptions);
...
function s3Url(options: S3Options) {
  let url: string;
  if (options.endpoint != null) url = `${options.endpoint}/${options.bucket}`;
