...
export function createHttpError(response: IncomingMessage, description: any | null = null) {
  return new HttpError(
    ...
    }\nHeaders: ${safeStringifyJson(response.headers)}`,
    description,
  );
}
...
export class HttpError extends Error {
  constructor(
    ...
    message: string = `HTTP error: ${HTTP_STATUS_CODES.get(statusCode) || statusCode}`,
...
export abstract class HttpExecutor<REQUEST> {
  static prepareRedirectUrlOptions(redirectUrl: string, options: RequestOptions): RequestOptions {
    ...
    if (headers?.authorization != null && (headers.authorization as string).startsWith("token")) {
      ...
      if (parsedNewUrl.hostname.endsWith(".amazonaws.com")) delete headers.authorization;
    }
    return newOptions;
  }
        ...
        requestOptions,
        {
          ...
          responseHandler: (response, callback) => {
            ...
            response.on("data", (chunk: Buffer) => {
              ...
              } else if (result == null) result = chunk;
              else {
  ...
  protected doDownload(requestOptions: any, options: DownloadCallOptions, redirectCount: number) {
    const request = this.createRequest(requestOptions, (response: IncomingMessage) => {
      if (response.statusCode! >= 400) {
        options.callback(
          new Error(
            `Cannot download "${requestOptions.protocol || "https:"}//${requestOptions.hostname}${
              requestOptions.path
  ...
  private handleResponse(
    ...
    if (response.statusCode === 404) {
      ...
      reject(
        createHttpError(
          response,
          `method: ${options.method || "GET"} url: ${options.protocol || "https:"}//${options.hostname}${
...
export class DigestTransform extends Transform {
  ...
  constructor(
    ...
    private readonly encoding: "hex" | "base64" | "latin1" = "base64",
  ) {
    super();
  ...
  validate() {
    if (this._actual == null) throw newError("Not finished yet", "ERR_STREAM_NOT_FINISHED");
...
function checkSha2(
  sha2Header: string | null | undefined,
  sha2: string | null | undefined,
  callback: (error: Error | null) => void,
): boolean {
...
export function configureRequestOptions(
  options: RequestOptions,
  token?: string | null,
  ...
  if (token != null) (headers as any).authorization = token.startsWith("Basic") ? token : `token ${token}`;

  if (headers["User-Agent"] == null) headers["User-Agent"] = "deskgap-builder";
  ...
  if (options.protocol == null && (process.versions as any).deskgap != null) options.protocol = "https:";
