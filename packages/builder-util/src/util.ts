...
if (process.env.JEST_WORKER_ID == null) sourceMapSupport.install();
...
function getProcessEnv(env: { [key: string]: string | undefined } | undefined | null): NodeJS.ProcessEnv | undefined {
  if (process.platform === "win32") return env == null ? undefined : env;
...
export function exec(
  ...
  if (log.isDebugEnabled) {
    ...
    if (options != null) {
      if (options.cwd != null) logFields.cwd = options.cwd;

      if (options.env != null) {
        const diffEnv = { ...options.env };
        for (const name of Object.keys(process.env)) if (process.env[name] === options.env[name]) delete diffEnv[name];
      ...
      (error, stdout, stderr) => {
        if (error == null) {
          if (isLogOutIfDebug && log.isDebugEnabled) {
            ...
            if (stdout.length > 0) logFields.stdout = stdout;

            if (stderr.length > 0) logFields.stderr = stderr;
          ...
          if (stdout.length !== 0) {
            if (file.endsWith("wine")) stdout = stdout.toString();

            message += `\n${chalk.yellow(stdout.toString())}`;
          }
          if (stderr.length !== 0) {
            if (file.endsWith("wine")) stderr = stderr.toString();
...
function logSpawn(command: string, args: string[], options: SpawnOptions) {
...
export function doSpawn(
  ...
  if (options.stdio == null) {
    ...
    options.stdio = [
      extraOptions?.isPipeInput ? "pipe" : "ignore",
...
export function spawnAndWrite(command: string, args: string[], data: string, options?: SpawnOptions) {
...
function handleProcess(
  ...
  childProcess.once(event, (code: number) => {
    if (log.isDebugEnabled) {
      ...
      if (out.length > 0) fields.out = out;
    ...
    } else reject(new ExecError(command, code, formatOut(out, "Output"), formatOut(errorOut, "Error output")));
  });
}
...
export function addValue<K, T>(map: Map<K, T[]>, key: K, value: T) {
  const list = map.get(key);
  if (list == null) map.set(key, [value]);
  else if (!list.includes(value)) list.push(value);
}

export function replaceDefault(inList: string[] | null | undefined, defaultList: string[]): string[] {
  if (inList == null || (inList.length === 1 && inList[0] === "default")) return defaultList;

  const index = inList.indexOf("default");
  if (index >= 0) {
    const list = inList.slice(0, index);
    list.push(...defaultList);
    if (index !== inList.length - 1) list.push(...inList.slice(index + 1));
...
export function getPlatformIconFileName(value: string | null | undefined, isMac: boolean) {
  ...
  if (!value.includes(".")) return `${value}.${isMac ? "icns" : "ico"}`;
...
export function executeAppBuilder(
  ...
  if (cacheEnv != null && cacheEnv.length > 0) env.ELECTRON_BUILDER_CACHE = path.resolve(cacheEnv);

  if (extraOptions.env != null) Object.assign(env, extraOptions.env);
  ...
  function runCommand() {
    return new Promise<string>((resolve, reject) => {
      ...
      if (childProcessConsumer != null) childProcessConsumer(childProcess);

      handleProcess("close", childProcess, command, resolve, (error) => {
        if (error instanceof ExecError && error.exitCode === 2) error.alreadyLogged = true;
  ...
  if (maxRetries === 0) return runCommand();
