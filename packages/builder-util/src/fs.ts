...
export interface FileConsumer {
  ...
  consume(file: string, fileStat: Stats, parent: string, siblingNames: string[]): any;
}

/**
 * Returns list of file paths (system-dependent file separator)
 */
export async function walk(initialDirPath: string, filter?: Filter | null, consumer?: FileConsumer): Promise<string[]> {
  let result: string[] = [];
  const queue: string[] = [initialDirPath];
  ...
  while (queue.length > 0) {
    ...
    let nodeModuleContent: string[] | null = null;
    ...
    for (const child of sortedFilePaths) if (child != null) result.push(child);

    dirs.sort();
    for (const child of dirs) queue.push(dirPath + path.sep + child);

    if (nodeModuleContent != null) result = result.concat(nodeModuleContent);
  }

  return result;
}

const _isUseHardLink =
  process.platform !== "win32" &&
  process.env.USE_HARD_LINKS !== "false" &&
  (require("is-ci") || process.env.USE_HARD_LINKS === "true");

export function copyFile(src: string, dest: string, isEnsureDir = true) {
  return (isEnsureDir ? ensureDir(path.dirname(dest)) : Promise.resolve()).then(() =>
    copyOrLinkFile(src, dest, null, false),
  );
}
 ...
 */
export function copyOrLinkFile(
  src: string,
  dest: string,
  stats?: Stats | null,
  ...
  if (isUseHardLink === undefined) isUseHardLink = _isUseHardLink;
  ...
  if (isUseHardLink)
    return link(src, dest).catch((e) => {
      if (e.code === "EXDEV") {
        ...
        if (isLog && log.isDebugEnabled) log.debug({ error: e.message }, "cannot copy using hard link");
...
export class FileCopier {
  ...
  constructor(
    ...
    if (isUseHardLinkFunction === USE_HARD_LINKS) this.isUseHardLink = true;
    else this.isUseHardLink = _isUseHardLink && isUseHardLinkFunction !== DO_NOT_USE_HARD_LINKS;
  }
  ...
  async copy(src: string, dest: string, stat: Stats | undefined) {
    ...
    if (this.transformer != null && stat != null && stat.isFile()) {
      ...
      if (data != null) {
        if (typeof data === "object" && "then" in data) data = await data;
    ...
    const isUseHardLink =
      afterCopyTransformer == null &&
      (!this.isUseHardLink || this.isUseHardLinkFunction == null
        ? this.isUseHardLink
        : this.isUseHardLinkFunction(dest));
    ...
    if (afterCopyTransformer != null) await afterCopyTransformer(dest);
  }
}
...
export function copyDir(src: string, destination: string, options: CopyDirOptions = {}): Promise<any> {
  ...
  return walk(src, options.filter, {
    consume: async (file, stat, parent) => {
      ...
      if (stat.isFile()) await fileCopier.copy(file, destFile, stat);
      else links.push({ file: destFile, link: await readlink(file) });
    },
