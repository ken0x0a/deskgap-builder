...
export function getArchCliNames(): string[] {
...
export function getArtifactArchName(arch: Arch, ext: string): string {
  ...
  if (arch === Arch.x64) {
    if (isAppImage || ext === "rpm") archName = "x86_64";
    else if (ext === "deb" || ext === "snap") archName = "amd64";
  } else if (arch === Arch.ia32) {
    if (ext === "deb" || isAppImage || ext === "snap") archName = "i386";
    else if (ext === "pacman" || ext === "rpm") archName = "i686";
