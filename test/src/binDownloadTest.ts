...
test("download binary from Mirror with custom dir", async () => {
  process.env.ELECTRON_BUILDER_BINARIES_MIRROR =  "https://github.com/deskgap-userland/deskgap-builder-binaries/releases/download/"
...
test("download binary from Mirror with Url override", async () => {
  process.env.ELECTRON_BUILDER_BINARIES_DOWNLOAD_OVERRIDE_URL =  "https://github.com/deskgap-userland/deskgap-builder-binaries/releases/download/linux-tools-mac-10.12.3"
  const bin = await getBinFromUrl("linux-tools", "mac-10.12.3", "SQ8fqIRVXuQVWnVgaMTDWyf2TLAJjJYw3tRSqQJECmgF6qdM7Kogfa6KD49RbGzzMYIFca9Uw3MdsxzOPRWcYw==")
